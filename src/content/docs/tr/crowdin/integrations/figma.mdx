...
<Steps>
  ...
  2. **Figma için Crowdin**’e tıklayın. The plugin opens on the **Settings** tab.
...
<Steps>
  ...
  4. *(İsteğe bağlı)* **Crowdin Dalı** bölümünde, içeriğinizin yükleneceği belirli dalı seçin. Eğer projeniz sürüm dallarını kullanıyorsa, ya varolanı seçebilir ya da doğrudan Figma’dan yeni bir tane oluşturabilirsiniz; aksi takdirde bu adımı atlayabilirsiniz.
...
<Steps>
  ...
  4. Görünen ileti öğesinde, eklenecek dizgilerin listesini gözden geçirin. Zaten bir Crowdin dizgisine bağlı olan öğeler <Icon name="mdi:link" class="inline-icon" /> ile işaretlenir. To add such an element as a new string instead of updating the linked one, click <Icon name="mdi:link" class="inline-icon" /> to unlink it, or click **Unlink all** to unlink all of them. Eğer gerekliyse, süzgeç panelini açmak ve belirli öğeleri hariç tutmak için süzgeç simgesine <Icon name="mdi:filter-variant" class="inline-icon" /> tıklayın:
  ...
  7. Sonraki ileti öğesi sayfasında tercih edilen seçenekleri seçin:
     * **Varolan dizgileri otomatik olarak bağlantıla** &ndash; Eklenen dizgileri Crowdin’deki varolan dizgilerle otomatik olarak karşılaştırır. If there's a match, the text element will be linked to the existing string instead of creating a new one.
     ...
     * **Kopyaları birleştir** &ndash; Crowdin’de aynı metin öğelerini tek bir yeni dizgiye bağlantılar. Örneğin, `Gönder` etiketli birkaç düğme eklerken, eklenti Crowdin projenize sadece bir dizgi ekler ve tüm kopya metin alanlarını buna bağlantılar.
  ...
  10. **Crowdin’e Gönder**’e tıklayın.
</Steps>
...
* **Oluşturulan dizgi**: Crowdin projenize eklenen yeni dizgilerin sayısı.
* **Güncellenen dizgi**: Crowdin’de güncellenmiş varolan dizgilerin sayısı.
* **Bağlantılı dizgi**: Crowdin’de Figma’daki öğelere bağlantılanmış varolan dizgilerin sayısı.
* **Bağlantılı Figma öğesi**: Crowdin’deki dizgilere bağlantılanmış Figma öğelerinin toplam sayısı. This number can be higher than **Strings linked** if multiple elements were linked to a single string.
...
<Steps>
  ...
  8. Tercih edilen seçenekleri ve Crowdin’de dizgileri eklemek istediğiniz dosyayı seçin ve ardından **Crowdin’e Gönder**’e tıklayın.
</Steps>
...
Figma’dan Crowdin projesine dizgiler eklemeyi basitleştirmek amacıyla kaynak dizgi tanımlayıcıları için istenen anahtar adlandırma şeklini ayarlayabilirsiniz. Figma için Crowdin eklentisi, seçilen şekle göre yeni dizgiler için dizgi tanımlayıcıları önerecek. Yeni kaynak dizgileri eklerken, önerilen tanımlayıcıyı her zaman tercih edilen görünüme göre düzenleyebilirsiniz.

Key naming is configured in the **String Defaults** panel, separately for the **Strings** and **Variables** tabs. Anahtar adlandırma şeklini seçmek için şu adımları izleyin:
...
<Steps>
  ...
  3. Açılan ileti öğesinde, dizgi listesinin üzerinde, sağ üst köşedeki <Icon name="mdi:cog-outline" class="inline-icon" /> simgesine tıklayın.
  4. **Anahtar Adlandırma Şekli** bölümünde, açılır menüden tercih edilen seçeneği seçin ve **Uygula**’ya tıklayın. The plugin regenerates the string keys in the list according to the selected pattern.
</Steps>

Varolan şekillerin yanı sıra kendi şeklinizi de yapılandırabilirsiniz. To use a custom pattern, select the **Custom key naming pattern** option from the drop-down list and specify your pattern in the **Custom Key Naming Pattern** field. Kullanılabilir yer tutucuları görmek için alana `[%` yazın.
...
Ek olarak, varsayılan snake_case (örn. `site_footer`) yerine camelCase (örn. `siteFooter`) içinde anahtarlar oluşturmak için **camelCase anahtar adlandırma stilini kullan** seçeneğini seçebilirsiniz.
...
## Figma Değişkenlerini Çevirme :badge[Yeni]{variant=success}

Eğer tasarımlarınız kullanıcı arayüzü kopyası için [Figma değişkenleri](https://help.figma.com/hc/articles/15339657135383) kullanıyorsa **Değişkenler** sekmesinde String değişkenlerini çevirebilirsiniz. In a variable collection, the default mode holds the source text, and each target language gets its own mode. Eklenti, varsayılan kip değerlerini kaynak dizgiler olarak Crowdin’e gönderir ve çevirileri tekrar dil kiplerine yazar. Bu değişkenleri kullanan tasarımlar daha sonra Figma’da yerel olarak çevrilmiş herhangi bir dile geçirilebilir.

Eklenti yerel String değişken koleksiyonlarıyla çalışır. It skips Color, Number, and Boolean variables, library variables, extended collections, and values that are aliases to other variables.
...
### Değişkenleri Crowdin’e Gönderme

<Steps>
  1. Figma için Crowdin eklentisini açın.
  2. **Değişkenler** sekmesine geçin.
  3. Çevirmek istediğiniz değişken koleksiyonlarını seçin ve **Crowdin’e Gönder**’e tıklayın.
  4. Eklenecek dizgilerin listesini gözden geçirin. Dizgi anahtarları koleksiyon, grup ve değişken adlarından oluşturulur. Zaten bir Crowdin dizgisine bağlantılı değişkenler <Icon name="mdi:link" class="inline-icon" /> ile işaretlenir ve bunların bağlantısını metin öğeleriyle aynı şekilde kaldırabilirsiniz. You can edit each string's key and context individually or click <Icon name="mdi:cog-outline" class="inline-icon" /> above the list to open the same **String Defaults** panel as in the **Strings** tab, where you can configure the [key naming pattern](#key-naming-pattern-settings) and the **Max. Length Correction (%)** option.
  ...
  6. **Dosya** açılır menüsünden Crowdin’de dizgileri eklemek istediğiniz dosyayı seçin ve **Crowdin’e Gönder**’e tıklayın. The button becomes available once a file is selected.
</Steps>
...
### Çevirileri Kipler İçine Eşitleme

**Çevirileri Eşitle**, seçilen koleksiyonlar Crowdin dizgilerine bağlantılı değişkenler içerdiğinde kullanılabilir hale gelir, bu yüzden önce değişkenleri Crowdin’e gönderin.

<Steps>
  1. **Değişkenler** sekmesinde gerekli koleksiyonları seçin ve **Çevirileri Eşitle**’ye tıklayın.
  2. Eşitlemek istediğiniz hedef dilleri seçin. Varsayılan olarak **Tüm diller** seçilidir.
  3. **Çevirileri Eşitle**’ye tıklayın.
</Steps>

For each selected language, the plugin creates or updates a mode named after the [Crowdin language code](/developer/language-codes/) (e.g., `uk`, `zh-CN`) in the selected collections and fills it with translations. Koleksiyonun varsayılan kipinin üzerine asla yazılmaz ve her zaman kaynak metninizi tutar.

<Aside>
  Koleksiyon başına kip sayısı Figma planınıza bağlıdır. Plan sınırını aşan diller atlanır ve eşitlemeden sonra bildirilir.
</Aside>
...
<ReadMore>
  [Figma Dev Mode](https://help.figma.com/hc/articles/15023124644247) hakkında daha fazlasını okuyun.
</ReadMore>
...
Dev Mode’da bulunmayan özellikler: Crowdin’e yeni dizgiler ekleme, dizgileri bağlantılama, ekran görüntüleri gönderme, çevirileri önizleme, dizgileri düzenleme, gizleme veya silme ve **Değişkenler** sekmesi.
...
<Steps>
  1. [Dev Mode](https://help.figma.com/hc/articles/15023124644247#01H8CR3K6VG21T3H9789QMV6CP)’a girin.
...
<ReadMore>
  [VS Kodu için Figma](https://help.figma.com/hc/articles/15023121296151) hakkında daha fazlasını okuyun.
</ReadMore>
