...
<Image src={webhooksAdd} alt="Tilføj Webhook" class="width-xl" />
...
Alternativt kan der føjes særlige headers til webhook'en. De kan bruges til ekstra sikkerhed som en godkendelsesmetode og meget mere. For example, if you add headers, your webhook endpoint can check them to ensure that information is coming from Crowdin.
...
Each event type in Crowdin has a specific payload format with the relevant event information. Webhook-indholdsdata kan tilpasses for at tilføje og organisere elementerne jf. aktuelle systemtkrav.

To check the event's possible variables, hover over the *Info* <Icon name="mdi:info-outline" class="inline-icon" /> icon in the *Payload* section's right upper corner.
...
<Image src={webhooksCallsHistory} alt="Kaldhistorik" />
