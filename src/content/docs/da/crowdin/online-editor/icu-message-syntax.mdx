...
Each argument is enclosed in single curly braces (e.g., `{variable}`) and refers to a value in the input data. Please note that using double curly braces (e.g., `{{variable}}`) is not supported and will cause syntax errors. Parametre af flg. typer understøttes i Crowdin: Nummer, dato, tid, vælg og flertal.
