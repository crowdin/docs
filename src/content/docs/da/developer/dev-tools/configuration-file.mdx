...
<CardGrid>
  <LinkCard title="VCS Integrations" description="GitHub, GitLab, Bitbucket, Azure Repos." href="/integrations/#vcs-integrations" target="_blank" />
  <LinkCard title="Konsolklient (CLI)" description="Cross-platform command-line tool." href="https://crowdin.github.io/crowdin-cli/" target="_blank" />
...
Kildefilerne er som standard tilgængelige for oversættelse til alle projektmålsprog. If some source files shouldn't be translated into specific target languages, you can exclude them with the help of the parameter `excluded_target_languages`.
...
Benyt flg. konstanter for at danne strukturen for CSV- eller XLS-/XLSX-filen:

* `identifier` &ndash; Column contains string identifiers.
* `source_phrase` &ndash; Column contains source strings.
* `source_or_translation` &ndash; Column contains source strings, but the same column will be filled with translations when the file is exported. Når eksisterende oversættelser uploades, bruges dataene fra denne kolonne som oversættelser.
...
* [Language mapping in Crowdin](/project-settings/languages/#adding-custom-language-codes)
* [Language mapping in Crowdin Enterprise](/enterprise/project-settings/languages/#language-mapping)
