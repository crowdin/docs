---
title: Sikkerhed
description: Configure security settings for your organization
slug: enterprise/security
sidebar:
  order: 1
---

import { Image } from 'astro:assets';
import { Steps, CardGrid, LinkCard, Aside } from '@astrojs/starlight/components';
import { Icon } from 'astro-icon/components';
import ReadMore from '~/components/ReadMore.astro';
import organizationSettingsMenuItem from '!/enterprise/organization-management/organization_settings_menu_item.png';
import customAuthAppCredentials from '!/enterprise/organization-management/custom_auth_app_credentials.png';

The **Security** page provides you with various security and authentication settings for your Crowdin Enterprise organization. You can set up how people can join your organization, what authentication methods can be used to log in, and manage API token and session policies.

To configure your security settings, follow these steps:

<Steps>
  1. Click on your profile picture in the upper-right corner and select **Organization Settings**. <Image src={organizationSettingsMenuItem} alt="Organization Settings Menu Item" />
  2. Switch to the **Security** section on the left sidebar.
</Steps>

## Sikkerhedsindstillinger

Configure advanced security settings such as **Allow Signup**, **Admin-Managed Invitations**, **Two-Factor Authentication**, **Device Verification**, personal token creation and lifetime, session timeouts, and **IP Allowlist**.

#### Allow Signup

Select whether people will be able to join the organization without prior invitation via email or a shareable link. Efter registreringen vil administratorer og projektledere kunne håndtere organisationsmedlemstilladelser.

#### Admin-Managed Invitations

Restrict the ability to invite new members to the organization to administrators only. When this option is enabled, project managers can still invite users who are already part of the organization to join their projects but won’t be able to add new users to the organization.

This option is useful when you want to maintain stricter control over who joins your organization, especially in regulated environments or when managing access centrally through a small group of administrators.

<Aside>
  **Allow Signup** and **Admin-Managed Invitations** are mutually exclusive. Only one of them can be enabled at a time.
</Aside>

#### Minimum Password Strength :badge[New]{variant=success}

Define the complexity requirements for user passwords to ensure account security. When a minimum level is set (e.g., **Medium**), all organization members must ensure their passwords meet the specified criteria to maintain access. This helps prevent the use of weak or easily guessable credentials, protecting both user accounts and organization data.

<ReadMore>
  Read more about [Password Strength Levels](/enterprise/account-settings/#password-strength).
</ReadMore>

#### Tofaktorgodkendelse

Two-factor authentication ensures an additional level of security for your Crowdin Enterprise organization. Når aktiveret, anmodes alle organisationsmedlemmer om at opsætte tofaktorgodkendelse på deres konti. Organisationsmedlemmer vil herefter i tillæg til et brugernavn og adgangskode skulle bruge en godkendelseskode ved hver indloning.

#### Enforce Security Keys :badge[New]{variant=success}

Require the use of hardware devices for user authentication. Enabling this option restricts the use of software-based methods like verification codes from authenticator apps, providing a higher level of protection against phishing attacks.

<Aside>
  To enable **Enforce Security Keys**, **Two-Factor Authentication** must be enabled first.
</Aside>

#### Restrict 2FA Reset Requests :badge[New]{variant=success}

Disable the self-service option for users to request a Two-Factor Authentication reset via the login page. When enabled, users who lose access to their 2FA device cannot initiate a reset themselves. Instead, an administrator must manually verify the user's identity and reset the 2FA settings.

#### Device Verification

Add an extra security step when users log in from a new device. Once enabled, Crowdin Enterprise will require device verification via email before granting access to your organization. This helps ensure that only authorized users can access your organization, even if their login credentials are compromised.

Device verification is triggered when:

* A user logs in from an unrecognized device.
* A user clears their browser cookies or switches browsers.

Device verification can be bypassed if:

* [Two-factor authentication](#two-factor-authentication) (2FA) is enabled and configured for the user’s account. In this case, device verification is not enforced since 2FA already provides strong authentication.
* Your organization uses external authentication through your own identity provider and doesn't rely on email verification.

#### Allow Personal Token Creation

Enable this option to allow organization members to generate their own personal access tokens for authenticating with the API. When disabled, members will not be able to create new tokens.

#### Maximum Token Lifetime

Set a maximum number of days a new token can be valid for. This policy applies only to tokens created after the setting is saved and does not affect previously created tokens. You can set a specific number of days (from 1 to 365) or select **Never expires**.

#### Idle Session Timeout

Automatically log out members after they have been inactive for a selected period.

* **20 minutes**: The shortest timeout. The **Remember me** option will not be available on the login page.
* **8 hours, 24 hours, 7 days, 30 days**: For users who click **Remember me** upon login, the system will log them out after this period of inactivity.

#### Session Lifetime :badge[New]{variant=success}

Configure an absolute time limit for session duration. Unlike **Idle Session Timeout**, which resets when the user is active, this setting requires re-authentication after the specified duration, regardless of activity.

* **Disabled**: Sessions do not have a fixed expiration time (default).
* **24 hours, 7 days, 30 days**: Users will be required to verify their identity again after this specific period.

#### IP-hvidliste

Grant access to your Crowdin Enterprise organization only to connections from allowed IP addresses.

With this feature, you can specify a list of IP addresses that can access your Crowdin Enterprise organization. For example, you can use IP Allowlist to grant access only to connections from your corporate network while blocking all other connections from IP addresses that don’t belong to your IP Allowlist. Once the IP Allowlist is enabled, Crowdin Enterprise starts filtering all connections both via UI and API.

<Aside>
  IP Allowlist is available only for the Business [subscription plan](https://crowdin.com/pricing#enterprise-plans) on Crowdin Enterprise.
</Aside>

To start using the IP Allowlist, first form the list of IP addresses and ranges that should have access to your organization using CIDR notation.

<ReadMore>
  Read more about [CIDR notation](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing#CIDR_notation).
</ReadMore>

IP-hvidlisten kan indeholde et ubegrænset antal IP-adresser og -områder, som kan tilgå Crowdin Enterprise-organisationen via UI og API.

Once you collected all of the IP addresses and ranges that should be added to the IP Allowlist, [Contact Support Team](https://crowdin.com/contacts) to enable the IP Allowlist for your organization. If you need to edit or remove some records from your IP Allowlist, contact Support Team with the respective request.

## Godkendelsesmetode

Afsnittet Godkendelsessmetoder muliggør at vælge, hvordan medlemmer kan logge ind i Crowdin Enterprise-organisationen. Ergo har folk stadig tid til at opsætte andre godkendelsesmetoder.

<Aside>
  Deaktiveres nogen af godkendelsesmetoderne, ophæves muligheden for at logge ind via denne specifikke metode efter 24 timer.

  Den pågældende notifikation vises også til alle organisationsmedlemmer, som bruger den ophævede godkendelsesmetode som information om nødvendigheden af at opsætte alternative metoder i Kontoindstillinger.
</Aside>

When you re-enable some of the earlier disabled authentication methods, they will immediately become available to the organization members.

* *Email/Username and Password* &ndash; members can use their email or Crowdin Enterprise username along with a password to log in.
* *Passkey* &ndash; members can log in using secure passkeys for passwordless authentication experience and enhanced account protection.
* *Magic Link* &ndash; members can request a one-time authentication link that will be sent to their emails.
* *Crowdin* &ndash; members can log in with their Crowdin account.
* *Facebook* &ndash; members can log in with their Facebook account.
* *GitHub* &ndash; members can log in with their GitHub account.
* *GitLab* &ndash; members can log in with their GitLab account.
* *Google* &ndash; members can log in with their Google account.
* *X* &ndash; members can log in with their X account.
* *Open ID Connect* &ndash; members can log in with their Open ID account.
* *SAML* &ndash; members can log in with their IDP account.

## Tilpassede Godkendelses-apps

The Facebook, GitHub, GitLab, Google, and X SSO allow you to use the default preconfigured Crowdin Enterprise global auth application, or you can set up your own custom auth application.

Følg disse trin for at opsætte en egen godkendelses-app:

<Steps>
  1. Click on your profile picture in the upper-right corner and select **Organization Settings**.
  2. Switch to the **Security** section on the left sidebar.
  3. Scroll down to the Authentication methods section and click on the needed SSO you'd like to set up the custom auth application for.
  4. Select **Custom auth application** and paste your credentials from your custom auth application and click **Save**. <Image src={customAuthAppCredentials} alt="Authentication Custom Auth App Credentials" />
  5. Copy <Icon name="mdi:content-copy" class="inline-icon" /> the **Redirect URL** from the **Auth Settings** page and paste it into your custom auth application settings.
</Steps>

Similar configuration flow is also applicable to the [OpenID Connect](/enterprise/oidc/) authentication method.

## Se også

<CardGrid>
  <LinkCard title="SAML Single Sign-On" description="Learn how to set up SAML SSO for your organization." href="/enterprise/saml/" />
  <LinkCard title="Terms Compliance App" description="Use this app to require users to accept your terms and conditions before they can access the organization." href="https://store.crowdin.com/terms-compliance" target="_blank" />
  <LinkCard title="Device Approvals App" description="Use this app to require admin approval for new devices before users can access the organization." href="https://store.crowdin.com/device-approvals" target="_blank" />
</CardGrid>
