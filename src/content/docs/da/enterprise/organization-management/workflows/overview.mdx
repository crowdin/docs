...
En arbejdsgang er en sekvens af trin, som projektindhold skal gennemgå, såsom oversættelse eller korrekturlæsning. You can use unlimited workflow steps that can be connected either in parallel or subsequently.

You can create a workflow template that works best for you and assign it to several projects. Editing a workflow after it's assigned to a project won't affect the original template. Alternativt kan ny arbejdsgang oprettes, hver gang et projekt oprettes.
...
<Steps>
  ...
  2. At the lower-right corner, click **Create**. <Image src={createWorkflow} alt="Create Workflow" />
  3. Navngiv arbejdsgangen og tilføj evt. en beskrivelse.
  4. Klik på **Opret**.
  5. Select workflow steps from the left panel and drag them to the working area.
  6. Connect steps in the order in which the content should flow between them. Trin kan forbindes parallelt eller sekventielt.
  ...
  8. When you're finished creating your workflow, click **Save**. <Image src={workflowCreation} alt="Workflow Creation" />
</Steps>
...
<QuestionAnswer title="How can I download an XLIFF with switched source language when using the Switch Source Language step in my workflow?">
  ...
  <Steps>
    ...
    2. Go to **Workflow** in the main menu and select the step that follows the Switch Source Language step.
    3. Go to **File** in the main menu and select **Export Filtered in XLIFF**.
  </Steps>
