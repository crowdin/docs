...
Crowdin's localization process is based on translating source strings to the defined target languages. Kildestrenge uploades til systemet i lokaliseringsfiler. Each unique source string first uploaded or added (for CSV and other formats supporting [string editing](/enterprise/string-management/#string-editing)) in Crowdin is considered a master string. Alle andre strenge identiske med hovedstrengen, men uploadet/oprettet senere, betragtes som dubletstrenge.

<Aside>
  In [string-based](/enterprise/creating-project/#string-based-project) projects, duplicate strings are managed using the **Show within a version branch (regular detection)** option, which is enabled by default and cannot be changed.
</Aside>
...
* **Show within a version branch (regular detection)** &ndash; duplicates will be hidden only between [version branches](/enterprise/version-management/).
...
Den foretrukne måde, hvorpå Crowdin Enterprise skal tælle ord i projektet, kan prædefineres. Konkret gælder det, hvorvidt HTML-tags skal tælles som almindelige ord eller ej. By default, Crowdin Enterprise considers HTML tags as regular words for most of the supported formats, excluding the following ones: HTML, Front Matter HTML, HAML, MD, Front Matter MD, XML, WEBXML, IDML, XLIFF, XLIFF 2.0, ADOC, DOCX, DITA.
...
<ReadMore>
  More information about Crowdin [Word Counter](/enterprise/word-counter/).
</ReadMore>

## Assign custom placeholder

Følg disse trin for at tildele den oprettet tilpasset pladsholder til et projekt:
...
<Steps>
  ...
  4. Click **Save**. <Image src={assignCustomPlaceholder} alt="Assign Custom Placeholder" class="width-xl" />
</Steps>
