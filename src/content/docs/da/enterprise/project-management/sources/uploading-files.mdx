...
Once you've [created a project](/enterprise/creating-project/), the next step would be preparing source files for upload. Crowdin Enterprise supports a wide range of [file formats](/enterprise/supported-formats/), including Android XML, iOS Strings, XML, JSON, etc. Når filerne er klargjorte, kan de føjes til det relevante Crowdin Enterprise-projekt til oversættelse.

Crowdin Enterprise tilbyder et par muligheder for at tilføje projektindhold:

* Manuelt via web-UI'en.
* Integrér de værktøjer, som allerede anvendes, i Crowdin Enterprise-projektet. Explore available integrations on the [Crowdin Store](https://store.crowdin.com/).
...
<Tabs syncKey="projectType">
  ...
  <TabItem label="String-based project">
    <Steps>
      ...
      4. Crowdin Enterprise forsøger automatisk at detektere, om kildeindhold eller oversættelser uploades. Verify the detection is correct and adjust the language settings if needed (e.g., English (source) should be selected for source files if the project's source language is English).
...
* **Update strings** &ndash; update strings with the same identifiers. Denne indstilling er som standard valgt.
* **Cleanup mode** &ndash; remove previously uploaded strings not present in the newly uploaded file. By default, this option is cleared.
...
<CardGrid>
  ...
  <LinkCard title="Versions Management" description="Learn how to manage different versions of your project." href="/enterprise/version-management/" />
  <LinkCard title="Import Settings" description="Configure how placeholders, duplicates, and word counts are handled." href="/enterprise/project-settings/import/" />
</CardGrid>
