...
<Aside>
  Some modules require a dedicated authentication type instead of `crowdin_app`. For example, apps with the [Workflow Step Type](/developer/crowdin-apps-module-workflow-step-type/#authentication) module must use the `crowdin_agent` authentication type.
</Aside>
