...
<div class="alert alert-warning"><b>Note!</b> Don't download the <code>iframe.js</code> file and serve it up from your app server directly. The <code>iframe.js</code> file must be served up by Crowdin for establishing of the cross-domain messaging bridge. The <code>iframe.js</code> file is only intended for use in an iframe inside Crowdin and does not work for standalone web pages.</div>
...
<table>
  ...
  <tr>
    <td><code>editor.source_language_id</code></td>
    <td>
      <p><strong>Type:</strong> <code>string</code></p>
  ...
  <tr>
    <td><code>editor.workflow_step.title</code></td>
    <td>
      <p><strong>Type:</strong> <code>string</code></p>
