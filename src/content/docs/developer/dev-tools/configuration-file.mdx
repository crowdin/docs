...
Each time the integration commits translations, it uses the following default commit message:
...
The `[ci skip]` tag is added by default so that syncing many files at once doesn't trigger a separate CI build for each one. Your CI still runs as expected once these changes are merged into your main branch through a pull request.

To add your own text to the commit message, use the `commit_message` parameter. By default, your text is appended below the default message (equivalent to setting `append_commit_message` to `true`):
...
To use only your own message instead of the default one, set `append_commit_message` to `false`:
...
You can use the `%original_file_name%` and `%language%` placeholders in your commit message to insert the file name and language.
