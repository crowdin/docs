...
Defines how In-Context is initialized.  
Acceptable values:
...
window.jipt.start();
```
Manually activates In-Context for the current page.  
Use this if you set `start_type` to `manual` or if you want to re-activate In-Context after stopping it.
