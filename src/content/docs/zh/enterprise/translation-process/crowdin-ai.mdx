...
Crowdin 企业版与顶级 AI 提供商集成，包括 OpenAI、Google Gemini、Microsoft Azure OpenAI 等，使您能够利用考虑不同级别的额外背景的高级 AI 翻译。 只需点击几下，使用[通过 AI 自动翻译](#auto-translation-via-ai)即可将这些译文应用到您的内容中。 此外，Crowdin Enterprise 中的 AI 还可用于[编辑器中的 AI 建议](#using-ai-in-the-editor)和 [AI QA 检查](#ai-qa-check)。
...
<Steps>
  ...
  3. 在出现的对话框中，设置提示词参数：
     * 选择提示词类型（即[自动翻译 & AI 建议](#auto-translation-via-ai)、[对齐](#ai-alignment)、[QA 检查](#ai-qa-check)或创建自定义类型）
     * 自定义类型名称（特定于自定义提示词类型）
     ...
     * 高级模式 &ndash; 单击**高级模式**，以更高精度编写您的提示词。 使用以下占位符插入语境：<Aside type="tip">单击占位符可将其复制，然后粘贴到提示词字段中。</Aside>
       ...
       * `%json%` &ndash; 当前句段的字符串集合，包括字符串标识符（键）、语境、文本、最大长度以及其他语言译文（如果指定了翻译占位符）。
       ...
       * `%projectName%` &ndash; Crowdin 企业版项目名称。
