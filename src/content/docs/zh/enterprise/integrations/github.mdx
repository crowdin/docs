---
title: GitHub 集成
description: 在您的 GitHub 仓库和 Crowdin 之间同步文件
slug: zh-CN/enterprise/github-integration
sidebar:
  order: 1
---
...
GitHub 集成允许您在 GitHub 仓库和 Crowdin 企业版项目之间同步文件。

<Aside>
  在设置和使用 GitHub 集成之前，您需要先在您的 Crowdin 企业版组织中[安装 GitHub 集成](https://store.crowdin.com/github)。
</Aside>

在基于文件的项目中，有两种可能的 GitHub 集成模式供您选择：

* **原文和译文模式** &ndash; 在 GitHub 仓库和 Crowdin 项目之间同步原文件和译文文件。
* **目标文件包模式** &ndash; 以所选格式从 Crowdin 企业版项目生成翻译文件并将其推送到您的 GitHub 仓库。 在此模式下，集成会推送翻译文件，但不会同步仓库中的源。 如果您在 Crowdin 企业版项目中执行源文本审阅并希望将更新的源文本添加到您的仓库中，您可以将源语言添加为目标语言，该语言将与翻译一起推送到您的仓库中。
...
所有已翻译和已审批的文件（或目标文件包）会自动合并到 GitHub 仓库的 `l10n` 分支中。

<Aside type="tip" title="试试 GitHub Action！">
  使用此集成的替代方法是官方的 [Crowdin Github Action](https://github.com/marketplace/actions/crowdin-action)，它允许您更灵活地配置工作流并完全控制安全性和对仓库的访问，因为您不需要在第三方服务中授权 GitHub 账户。
</Aside>
...
<Steps>
  ...
  2. 在集成列表中单击 **GitHub**。
  3. 单击**设置集成**，然后从下拉列表中选择**原文和译文模式**或**目标文件包模式**，以通过您的 GitHub 账户进行集成。 <Image src={setUpIntegrationDropdown} alt="Set Up Integration button with options" class="width-sm" />
     <Aside>在基于字符串的项目中，GitHub 集成仅限于**目标文件包模式**运作。</Aside>
  4. 然后在 GitHub 端授权与 Crowdin 的连接：<Image src={authorization} alt="GitHub Integration Authorization" class="width-sm" />
</Steps>
...
<Tabs syncKey="projectType">
  <TabItem label="基于文件的项目">
    <Image src={unconfigured} alt="未配置 GitHub 集成" />

    建议将重复字符串切换为**在版本分支内显示**，这样相同的字符串将被隐藏在分支之间。 如果您的源文件包含具有明显标识符（键）的字符串，最好使用此选项的*严格*版本。 在其他情况下，请随意使用*常规*版本。
  </TabItem>
  <TabItem label="基于字符串的项目">
    <Image src={unconfiguredSb} alt="未配置 GitHub 集成" />
  </TabItem>
</Tabs>

<CardGrid>
  <LinkCard title="重复的字符串" href="/zh/enterprise/project-settings/import/#重复字符串" />
  <LinkCard title="版本管理" href="/zh/enterprise/version-management/" />
</CardGrid>

在**目标文件包模式**下使用 GitHub 集成时，集成将从您的 Crowdin 企业版项目发送已完成的译文，而无需从您的仓库中提取源文件。 因此，在选择应翻译的仓库和分支时，您可以指定集成应将生成的带有翻译的包放在何处。
...
设置集成时，您可以选择要添加到 Crowdin 企业版项目的现有仓库分支。 要自动将未来的分支从 GitHub 添加到 Crowdin 企业版，请在集成设置中为分支名称创建模式。
...
为使集成正常工作，您需要指定应翻译哪些源文件以及 Crowdin Enterprise 应如何在您的仓库中构建翻译文件。 要使集成在目标文件包模式下正常工作，您需要选择要推送到存储库的所需包。
...
<ReadMore>
  查看[在线配置 VCS 集成](/zh/enterprise/configuring-vcs-integrations-online/)，了解如何在线选择同步内容。
</ReadMore>

### 创建配置文件

配置文件 `crowdin.yml` 应与您想要翻译的每个独立分支一起存储在 GitHub 仓库中。
...
<Image src={syncNow} alt="GitHub 立即同步" />

或者，如果您只需要单独同步一个分支，请单击所需分支上的 <Icon name="mdi:dots-vertical" class="inline-icon" /> 并选择**同步分支**。

## 从仓库中上传译文

默认情况下，存储在仓库中的翻译仅在第一次同步时上传到 Crowdin 企业版。 要手动将翻译上传到 Crowdin Enterprise，请单击**立即同步**按钮旁边的 <Icon name="mdi:dots-vertical" class="inline-icon" /> 图标，然后单击**将翻译同步到 Crowdin Enterprise**。 该集成会将现有翻译上传至您的 Crowdin 企业版项目。
...
<QuestionAnswer title="如果我的仓库加入了 GitHub 组织，我该怎么办？">
  要继续在 Crowdin 企业版中使用 GitHub 集成，请按照以下步骤操作：

  1. 在您的 GitHub 账户中前往[已授权的 OAuth 应用](https://github.com/settings/applications)。
  2. 在列表中找到 **Crowdin** OAuth App 并撤销其访问权限。
  3. 使用上文所述的指引再次在您的 Crowdin 企业项目中设置 GitHub 集成。
</QuestionAnswer>

<QuestionAnswer title="如何避免因 Crowdin Enterprise 的提交触发不必要的应用程序构建？">
  由于 Crowdin Enterprise 中的 VCS 集成使用 API 而非 Git 来提交文件，因此每个文件都是单独提交的。
