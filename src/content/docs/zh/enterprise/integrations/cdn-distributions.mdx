...
<Tabs syncKey="projectType">
  <TabItem label="基于文件的项目">
    <Steps>
      ...
      6. 单击 **创建**。
      7. 复制 <Icon name="material-symbols:file-copy" class="inline-icon" /> **分发哈希值**和**分发清单**。 清单是一个 JSON 文件，包含关键的分发详情（例如，文件、语言、路径），可用于您的集成。
    </Steps>
  </TabItem>
...
<Aside>
  CDN 会将发布中的所有译文缓存长达 1 小时，因此即使在 Crowdin Enterprise 中发布了新的译文，CDN 也可能会延迟返回它们。
</Aside>
...
您可以在**组织设置 > 账单 > CDN 分发**中管理 CDN 使用价格并设置每日使用限制通知。
