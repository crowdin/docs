...
<Image src={editIntegrationEditBranchBtn} alt="编辑集成" class="width-2xl" />
...
<Image src={ignoreFiles} alt="忽略文件或文件夹" class="width-2xl" />
...
<Steps>
  1. 点击 **语言映射**。 <Image src={languageMapping} alt="语言映射" class="width-2xl" />
  2. 选择所需的语言和占位符。
  3. 指定您的自定义代码。
...
<Image src={configuringSpreadsheets} alt="配置电子表格" class="width-2xl" />

### 配置 XML 文件

您可以配置 XML 文件导入设置，以指定 Crowdin 企业版应如何处理 XML 文件。

<Image src={configuringXml} alt="配置 XML 文件" class="width-2xl" />
...
<Steps>
  1. 点击 **保存更改** 以预览创建的配置。 <Image src={filesListFilter} alt="保存配置" class="width-2xl" />
  2. 如果您有多个具有不同配置的文件组，请点击 **添加文件过滤器**。 <br />如果项目中有多个分支具有相同的配置，且您希望对它们应用相同的过滤器，请选择 **将过滤器应用于所有可翻译分支**。
  3. 点击 **保存** 以保存创建的配置。 <Image src={filesList} alt="将过滤器应用于所有可翻译分支" class="width-2xl" />
</Steps>
