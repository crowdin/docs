---
title: Bitbucket 集成
description: 在您的 Bitbucket 存储库和 Crowdin 之间同步文件
...
Bitbucket 集成允许您在 Bitbucket 仓库和 Crowdin 企业版项目之间同步文件。

<Aside>
  您需要在 Crowdin Enterprise 组织中[安装 Bitbucket 集成](https://store.crowdin.com/bitbucket)，然后才能设置和使用它。
</Aside>

在基于文件的项目中，有两种可能的 Bitbucket 集成模式可供选择：

* **源文件和翻译文件模式** &ndash; 在您的 Bitbucket 仓库和 Crowdin Enterprise 项目之间同步源文件和翻译文件。
* **目标文件包模式** &ndash; 以所选格式从 Crowdin 企业版项目生成翻译文件并将其推送到您的 Bitbucket 仓库。 在此模式下，集成会推送翻译文件，但不会同步仓库中的源。 如果您在 Crowdin 企业版项目中执行源文本审阅并希望将更新的源文本添加到您的仓库中，您可以将源语言添加为目标语言，该语言将与翻译一起推送到您的仓库中。
...
所有已翻译和已批准的文件（或目标文件包）将自动合并到 Bitbucket 存储库的 `l10n` 分支中。

## 连接 Bitbucket 账户

<Steps>
  1. 打开您的项目，然后在左侧边栏中选择**集成**。
  2. 在集成列表中单击 **Bitbucket**。
  3. 点击**设置集成**，然后从下拉列表中选择**源文件和翻译文件模式**或**目标文件包模式**，以通过您的 Bitbucket 账户进行集成。 <Image src={setUpIntegrationDropdown} alt="Connecting Bitbucket" class="width-sm" />
     <Aside>在基于字符串的项目中，Bitbucket 集成仅限于**目标文件包模式**运作。</Aside>
  4. 然后在 Bitbucket 端授权与 Crowdin 的连接：<Image src={authorization} alt="Authorization" class="width-lg" />
</Steps>
...
在**目标文件包模式**下使用 Bitbucket 集成时，集成将从您的 Crowdin Enterprise 项目发送已完成的翻译，而无需从您的仓库中提取源文件。 因此，在选择应翻译的仓库和分支时，您可以指定集成应将生成的带有翻译的包放在何处。
...
设置集成时，您可以选择要添加到 Crowdin 企业版项目的现有仓库分支。 要将未来的分支从 Bitbucket 自动添加到 Crowdin 企业版，请在集成设置中为分支名称创建一个模式。
...
配置文件 `crowdin.yml` 应与您要翻译的每个单独分支一起存储在 Bitbucket 仓库中，以便 Crowdin Enterprise 知道应将哪些文件发送以进行翻译。
...
<Image src={syncNow} alt="立即同步 Bitbucket" />
