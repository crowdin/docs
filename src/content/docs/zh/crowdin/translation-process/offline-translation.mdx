...
<Steps>
  1. 打开您的项目并选择一种语言。
  2. 单击您想下载的文件旁边的 <Icon name="mdi:dots-horizontal" class="inline-icon" />。
  3. 选择**下载**或**以 XLIFF 格式导出**。 <Image src={downloadFiles2} alt="下载文件" />
</Steps>
...
<Steps>
  1. 打开您的项目并选择一种语言。
  2. 单击 <Icon name="heroicons:arrows-up-down-solid" class="inline-icon" />。
  3. 选择**下载译文**或**以 XLIFF 格式导出**。 <Image src={downloadFiles} alt="下载所有文件" />
</Steps>
...
如果您处于编辑器的[多语言（网格）](/zh/online-editor/#multilingual-mode-grid)模式下，请单击目标语言列旁边的下拉箭头，然后选择**以 XLIFF 格式导出筛选内容**。 <Image src={downloadUploadGrid} alt="从多语言（表格）模式导出筛选后的字符串" />

<ReadMore>
  详细了解[在线编辑器](/zh/online-editor/)。
</ReadMore>
...
如果您处于编辑器的[多语言（表格）](/online-editor/#multilingual-mode-grid)模式，请点击目标语言列旁边的下拉箭头，然后选择**上传译文**。 <Image src={downloadUploadGrid} alt="从多语言（表格）模式上传译文" />
...
<Steps>
  1. 打开您的项目并选择一种语言。
  2. 单击您想上传译文的文件旁边的 <Icon name="mdi:dots-horizontal" class="inline-icon" />。
...
<Steps>
  1. 打开您的项目并选择一种语言。
  2. 单击 <Icon name="heroicons:arrows-up-down-solid" class="inline-icon" />。
  3. 选择**上传 XLIFF 译文**。 <Image src={uploadTranslationsXliff} alt="以 XLIFF 格式上传译文" />
  4. 根据需要调整[高级导入设置](#advanced-import-settings)。
  5. 点击**选择文件**，从您的设备中选择包含译文的 XLIFF 文件。
</Steps>

<QuestionAnswer title="我的 XLIFF 文件已离线翻译完成。 现在它包含完成的译文，但当我尝试将其上传回 Crowdin 时，译文并没有显示在项目中。 可能是什么原因？">
  ...
  * `final` – 该句段已翻译并获批准。
  ...
  但是，如果您的 CAT 工具不会自动更改已翻译字符串的 `state` 属性，或者翻译是在记事本中手动完成的，则需要手动将 XLIFF 文件中的 `state` 属性更改为 `translated` 或 `final` 值，以确保译文能够成功上传到 Crowdin。
</QuestionAnswer>
