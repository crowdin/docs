...
<Steps>
  ...
  2. 在所需文件上单击 <Icon name="mdi:dots-horizontal" class="inline-icon" />（或右键单击），然后选择 **Settings**。 <Image src={fileContextMenuSettings} alt="文件上下文菜单" />
  3. 在弹出的对话框中，切换到 **Parser configuration** 选项卡。
  4. 选择 **Enable content segmentation** 和 **Use custom segmentation rules**。
  5. 粘贴您的 SRX 分段规则，然后单击 **Save**。 <Image src={fileParserConfiguration} alt="文件分段规则" class="width-xl crowdin-modal" />
</Steps>
