  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <p><strong>描述：</strong> 包含用于包生成的翻译字符串。可使用 <code>strings</code>（内联数组）或 <code>stringsUrl</code>（NDJSON 的公开链接）。<br /><Aside type="tip">处理大型负载或多语言导出时，请使用 <code>stringsUrl</code> 字段以避免超出大小限制。</Aside><br />
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <p><strong>描述：</strong> 使用 <code>data.content</code>（base64 编码）或 <code>data.contentUrl</code>（可公开访问的 URL）返回生成的包文件。响应中只应包含其中一个。</p><p>文件格式取决于您的实现。</p><p><Aside title="限制">最大响应负载大小为 5 MB。</Aside></p>
      </td>
    </tr>
    <tr>
