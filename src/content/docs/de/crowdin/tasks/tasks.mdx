...
To split content between several members select **Split files** (in file-based project) or **Split strings** (in string-based project) in the **Assign users** dialog. You can set the approximate amount of words for each assignee.
...
<QuestionAnswer title="How do the source file updates affect the existing translation and proofreading tasks?">
  ...
  * The newly added strings won't affect the existing task in any way.
  ...
  Wenn die Quelldatei auf die Revision zurückgesetzt wird, die die entfernten Zeichenfolgen enthält, werden diese wieder in der Aufgabe angezeigt.
</QuestionAnswer>
...
<CardGrid>
  ...
  <LinkCard title="Crowdin Language Services" description="Order professional translations and proofreading services." href="/crowdin-language-services/" />
</CardGrid>
