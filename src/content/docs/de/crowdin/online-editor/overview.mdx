...
Der Editor ist der zentrale Bereich in Crowdin, in dem Projektmitglieder an Übersetzungen arbeiten können. It allows users to suggest, vote on, review, and approve translations online.
...
| <Icon name="mdi:image-outline" class="inline-icon" /> **Screenshots**                  | The string has screenshots attached.                          | Öffnet den Bereich **Kontext & Übersetzungen** und den ersten Screenshot im [Screenshot-Viewer](#screenshot-viewer). |
...
<Aside>
  Stelle sicher, dass du [QA-Prüfungen](/project-settings/qa-checks/) aktivierst, damit du Strings findest, die überprüft werden müssen. Issues found will be highlighted in red under the translated strings that require revision.
</Aside>
...
* *Hidden* &ndash; Show strings that are hidden from translators. Strings können von Projektmanagern ausgeblendet oder von Crowdin automatisch ausgeblendet werden, wenn sie als Duplikate markiert sind.
...
Einige Zeichenketten könnten Pluralformen enthalten. Depending on the language, the number of plural forms may vary. For example, Chinese has one plural form, English has two plural forms, and other languages may have up to six plural forms. Crowdin verarbeitet Pluralformen gemäß den CLDR-Regeln für Sprachpluralformen.
...
* **Bestätigung entfernen** &ndash; Entfernt die Bestätigung einer Übersetzung und setzt ihren Status auf nicht bestätigt zurück. (Available for proofreaders or higher.)
...
Some terms may already have translations, which help maintain consistency across the project. Um einen übersetzten Begriff wiederzuverwenden, klicke auf den unterstrichenen Text. Die Übersetzung wird dann automatisch im Übersetzungsfeld angezeigt. Alternativ kannst du den Mauszeiger über den unterstrichenen Text bewegen, dann über die benötigte Begriffsübersetzung in der Karte und auf **Einfügen** klicken. Verwende diese Option, wenn für den Begriff mehrere Übersetzungen vorhanden sind und du eine bestimmte auswählen möchtest.
...
The *File Context* section allows you to provide translators with additional details to help them understand how to translate a specific file. Dateikontext kann als einfacher Text oder Markdown entweder direkt im Editor oder über die [Dateieinstellungen](/file-management/#file-context) von Projektmitgliedern mit Manager-Berechtigungen (oder höher) hinzugefügt werden. For other project members, file context is read-only.

To add file context in the Editor, follow these steps:

<Steps>
  1. Click **Add File Context** in the File Context section.
  2. Add the context or instructions for your translators. You can use either plain text or Markdown.
...
Use keyboard shortcuts to take actions in the editor quickly. Check the list of keyboard shortcuts by clicking the *keyboard* <Icon name="mdi:keyboard" class="inline-icon" /> icon in the upper-right corner or selecting **Help > Keyboard Shortcuts** in the main menu. Alternatively, press <KBD>Ctrl</KBD>+<KBD>Shift</KBD>+<KBD>K</KBD> or <KBD>Shift</KBD>+<KBD>⌘</KBD>+<KBD>K</KBD>. Check the list of keyboard shortcuts by clicking the <em x-id="3">keyboard</em> <img src="{{site.url}}/assets/material_icons/keyboard.png" style="box-shadow:none" class="my-0 mx-1" mark="crwd-mark" /> icon in the right upper corner. Click on the necessary key combination, and modify it with the help of your keyboard.
...
<Steps>
  ...
  2. In the **Auto-Translate strings** dialog, configure the following parameters:
     ...
     * **Allow duplicate translations** – adds translations even if they duplicate existing ones. Nützlich zum Vergleichen von KI-Prompts oder MT-Engines im Bericht [Genauigkeit der automatischen Übersetzung](/project-reports/#auto-translation-accuracy).
...
* **Duplicates** – Filter strings based on their duplicate status. Options include *Master Strings*, *Duplicates Only*, *Duplicates with Shared Translations*, and *Duplicates with Own Translations*. <Aside>Diese Option wird nur angezeigt, wenn [**Doppelte Strings**](/project-settings/import/#duplicate-strings) so konfiguriert ist, dass Duplikate automatisch übersetzt werden. Shared translations come from the master string, while own translations are entered directly on the duplicate.</Aside>
...
<LinkCard title="Expression Syntax Elements" description="Learn how to build and use verbal expressions with practical examples." href="/expression-syntax-elements/" />
