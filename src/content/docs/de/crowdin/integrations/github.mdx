...
* **Target File Bundles Mode** &ndash; Übersetzungsdateien im ausgewählten Format aus dem Crowdin-Projekt generieren und in dein GitHub-Repository übertragen. In this mode, integration pushes translation files and doesn't sync sources from your repo. In this mode, integration pushes translation files and doesn't sync sources from your repo.
...
Bei der Arbeit mit der GitHub-Integration im Modus Ziel-Dateibundles sendet die Integration die abgeschlossenen Übersetzungen aus deinem Crowdin-Projekt, ohne Quelldateien aus deinem Repository abzurufen. So when selecting a repository and branches that should be translated, you specify where the integration should put the generated bundles with translations.
...
When you set up the integration, you select existing repository branches to be added to the Crowdin project. Um zukünftige Branches von GitHub automatisch zu Crowdin hinzuzufügen, erstelle in den Integrationseinstellungen ein Muster für die Branchnamen.
...
<QuestionAnswer title="How can I avoid unnecessary application builds triggered by commits from Crowdin?">
  ...
  * By default, the `[ci skip]` tag is added to the commit message to skip unnecessary builds. Read more about the [Commit Message parameter](/developer/configuration-file/#commit-message).
