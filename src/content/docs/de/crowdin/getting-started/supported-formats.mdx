...
<ReadMore>
  Erfahre mehr über die [Asset-Lokalisierung](/asset-localization/).
</ReadMore>
...
By default, we export the translations in the same format as the source files. So, for example, if you upload an XML file to Crowdin, you'll have the XML file exported.

Wenn du Übersetzungen in einem anderen Format als dem Quellformat exportieren möchtest, verwende [Target File Bundles](/bundles/).

Our developers can create a special processor if you want to configure custom settings for your file export. Du lädst beispielsweise eine Schlüssel-Wert-Datei hoch, die standardmäßig keine [Zeichenfolgenbearbeitung](/string-management/#string-editing) unterstützt, und möchtest sie direkt in Crowdin bearbeiten. You upload a key-value file that doesn’t support <a href="{%link _docs_enterprise/file-management.md %}#string-editing">string editing</a>\{:target="_blank"} by default and want to modify it directly in Crowdin Enterprise. The exported file will remain in the original format.
