...
Also, you can perform [Auto-Translation](/enterprise/auto-translation/) using MT engines.
...
* **Style rules**: Applies a writing style defined in your DeepL account. Works only when the target language matches the language of the style rule.
* **Translation memories**: Reuses previously approved translations from a DeepL translation memory for matching segments.
...
<Aside type="note">
  The **Tag handling version**, **Style rules**, **Translation memories**, and **Custom instructions** options appear once you select the **Prefer quality optimized** or **Quality optimized** model type. Style rules, translation memories, and custom instructions also require a DeepL API Pro key and apply only to the language pairs that next-generation models cover. Translations that fall back to the classic models do not use them.
</Aside>

#### DeepL Resource Configuration

The DeepL Translator can use glossaries, style rules, and translation memories from your connected DeepL account to keep your terminology and translations consistent. In Crowdin Enterprise, you can configure several resources of each type and assign specific languages or language pairs to each one.
...
<Steps>
  ...
  3. *(Optional)* To configure another resource of the same type, click <Icon name="mdi:plus-circle" class="inline-icon" /> and repeat the steps above. To remove a resource configuration, click <Icon name="mdi:bin" class="inline-icon" />.
  4. Click **Create**.
</Steps>
...
<Steps>
  ...
  5. *(Optional)* Specify your **Resource location** (e.g., us-central1). The **Custom model** and **Glossary** drop-down lists are populated once your credentials and resource location are in place.
  6. *(Optional)* Select your **Custom model**. <Aside>If no Google Cloud AutoML custom model is specified in the MT engine settings, the Google NMT model will be used instead.</Aside>
  7. *(Optional)* Select your **Glossary** from the respective drop-down list.
  8. *(Optional)* Map the **Custom model language** to the corresponding **Crowdin Language**.
...
Depending on your [*HTML Tags Displaying* setting](/enterprise/online-editor/#appearance) and the file format, the Editor may show inline tags in a simplified, numbered form (for example, `<0>Sample</0>` instead of `<a href="https://sample.com">Sample</a>`). This is only a display option in the Editor. It is not the form that Crowdin Enterprise passes to the machine translation engine.
...
When MT engines are updated with new features or models, cached translations in Crowdin Enterprise may no longer reflect the latest translation capabilities. To display updated translations in the Editor’s **Automated Suggestions** section, clear the MT engine cache. This can be especially useful if you've already viewed certain strings in the Editor, as Crowdin Enterprise stores cached MT suggestions for those strings.
