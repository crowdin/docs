...
* Current scope &ndash; the number of words currently included in the task. If it differs from the original scope, hover over the <Icon name="mdi:help-circle-outline" class="inline-icon" /> icon to see how much it has changed. In a [task shared by a client](/enterprise/tasks/#shared-tasks), it grows until the content finishes synchronizing.
* Created &ndash; when the task was created.
