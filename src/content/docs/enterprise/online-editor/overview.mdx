...
Once saved, your suggestion will be added to the string's translation suggestions list, and you'll automatically move on to the next string.
...
If issues are detected when you attempt to save a translation or when Auto-save triggers upon navigating to the next string, an inline **Review issues to save** banner will appear below your translation. An orange warning badge will also appear on the **Save** button.

<Image src={qaInlineBanner} alt="QA issues inline banner" />

Depending on how QA checks are configured in the project settings, the Editor handles detected issues in two ways:

* **Errors (Blocking Issues):** Saving the translation and navigation (e.g., changing target languages, opening a different file, or applying filters) are prevented until you resolve the issue. Hovering over the **Save** button will display a "Resolve blocking issues first" tooltip. Note that errors are strictly enforced and will trigger the banner even if you disabled the **QA issues** option in your personal Editor settings. You can still switch to translate another string, but the translation for the current string will remain unsaved until the error is fixed.
* **Warnings (Non-blocking Issues):** For less critical inconsistencies, the behavior depends on whether the **QA issues** option is enabled in your [Editor Settings](#general):
  * **Setting ON:** The inline banner displays the warnings. Navigation is blocked until you resolve the issues or click **Save Anyway**.
  * **Setting OFF:** The translation saves automatically (if Auto-save is enabled) without displaying the banner, bypassing these warnings entirely.
...
- *Auto-save translations* – if enabled, your translations are automatically saved when you navigate between strings.
