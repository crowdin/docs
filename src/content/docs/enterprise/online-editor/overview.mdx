...
Using the main menu at the top of the Editor (**File**, **Edit**, **View**, **Workflow** (only in [projects with a workflow](/enterprise/creating-project/#projects-with-a-workflow)), **Language**, **Project**, **Help**), you can select content for translation, download or upload translations, perform various string actions, change the Editor's layout, switch target languages, access project pages and switch projects, access help materials, and more.
...
In [projects with a workflow](/enterprise/creating-project/#projects-with-a-workflow), strings can additionally have the following workflow statuses:

- <Icon name="todo" size="24" class="inline-icon align-middle" /> **Todo** – Indicates that the string is awaiting action at the current workflow step. For example, on a **Translation** step, it means the string is untranslated. On a **Proofreading** step, it means the string is not yet approved.
- <Icon name="pending" size="24" class="inline-icon align-middle" /> **Pending** – This status is specific to the [**Source Text Review**](/enterprise/source-text-review/) step. It indicates that a string has an approved correction with text that differs from the original source text.
- <Icon name="done" size="24" class="inline-icon align-middle" /> **Done** – Indicates that the main action for the current workflow step is complete. For example, on a **Translation** step, the string has been translated. On a **Proofreading** step, it has been approved.
...
| <Icon name="mdi:image-outline" class="inline-icon" /> **Screenshots**                  | The string has screenshots attached.                          | Opens the **Context & Translations** panel and the first screenshot in the [screenshot viewer](#screenshot-viewer). |
...
To streamline the translation process, you can enable the Auto-save feature. When the **Auto-save translations** setting is enabled in the [Editor Settings](#general), your translations are automatically saved when you switch to another string. A "Saving..." indicator will appear briefly, followed by a "Saved" confirmation.

By default, Auto-save is disabled, meaning you need to manually click <Icon name="mdi:content-save" class="inline-icon" /> **Save translation** for each string. When Auto-save is disabled, the **Save translation** button acts as the primary action and appears green. When Auto-save is enabled, the button becomes gray, as manual saving is optional.

Once saved, your suggestion will be added to the string's translation suggestions list. By default, the **Automatically move to next string** option is enabled, so the Editor will then move to the next string.

If you need to discard your current input and revert to the previous state before saving, click <Icon name="material-symbols:undo" class="inline-icon" /> **Cancel**.
...
- <Icon name="mdi:pencil" class="inline-icon" /> - *Highlight untranslated, translated, and approved strings*. Use it to toggle the color highlighting of strings on and off.
...
* *All translations* – search through every translation of a string.
* *Final translations* – search only the translation shown at the top of the translation list and used on download.
...
You can combine *Match case* with either *Match whole phrase* or *Exact match*. Note that *Match whole phrase* and *Exact match* are mutually exclusive. Enclosing the search phrase in quotation marks enables *Match whole phrase* automatically, unless *Exact match* is selected.

The search phrase is limited to 128 characters. If you use a search phrase longer than 128 characters, it will be automatically truncated to the maximum allowed length.

You can also find a specific string by its internal numeric identifier (a number automatically assigned by Crowdin). To do so, type the numeric ID (for example, `34354`) into the search field without quotation marks to get an exact match. A string ID search ignores the filters and the search options.
To search for the occurrences of the number in the text or context, enclose it in quotation marks (for example, `"34354"`) or select **Search as text**.
...
* *Machine Translations* &ndash; Show strings that are translated by TM, MT, or AI suggestions without changes. Often these strings require additional review.
...
To help you maintain translation quality and prevent common inaccuracies, the Editor automatically performs Quality Assurance (QA) checks. These checks scan for issues like punctuation discrepancies, spacing mismatches, missing placeholders, or inconsistent terminology.

The Editor's behavior depends on whether these issues are configured as **Errors** or **Warnings**:

* **Errors (Blocking Issues):** The inline **Review issues to save** banner appears, and an orange warning badge appears on the **Save translation** button. Saving the translation and navigation (e.g., changing target languages, opening a different file, or applying filters) are prevented until you resolve the issue. Hovering over the **Save translation** button displays a "Resolve blocking issues first" tooltip. Errors are strictly enforced and trigger the banner even if you disabled the **QA issues** option in your personal Editor settings. You can still switch to translate another string, but the translation for the current string will remain unsaved until the error is fixed.
* **Warnings (Non-blocking Issues):** For less critical inconsistencies, the behavior depends on whether the **QA issues** option is enabled in your [Editor Settings](#general):
  * **Setting ON:** The inline **Review issues to save** banner displays the warnings, and an orange warning badge appears on the **Save translation** button. Navigation is blocked until you resolve the issues or click **Save Anyway**.
  * **Setting OFF:** The translation saves automatically (if Auto-save is enabled) without displaying the banner or warning badge, bypassing these warnings entirely.

<Aside>
  Project managers define whether a specific QA check acts as an Error or a Warning. However, some critical checks (e.g., syntax errors) are permanently locked as Errors to prevent severe translation issues.
</Aside>

If QA issues are detected when you manually save a translation or when Auto-save triggers upon switching strings, they will be displayed in the inline banner. Below is an example of a punctuation mismatch (configured as a Warning by default):

<Image src={qaInlineBanner} alt="QA issues inline banner" />

#### Auto-fixing QA Issues

For many common errors, the inline QA banner provides automatic quick-fixes:

* Click the <Icon name="mdi:magic-staff" class="inline-icon" /> **Autofix** icon next to an individual issue to apply the correction automatically.
* Click **Fix (N)** in the banner header to instantly apply all available auto-fixes for the current string.

Once the issues are resolved, the translation will automatically save and allow you to continue.
...
* **Remove My Approvals / All Approvals** &ndash; Remove the approvals for a translation, reverting its status to unapproved. (Available for proofreaders or higher.)
...
- AI suggestions (if enabled in the [project settings](/enterprise/project-settings/ai/))
...
Screenshots tagged with the string appear as preview cards in the **Context & Translations** section. Each card shows the whole screenshot with a red outline around the place where the string is used. Hover over a card for an enlarged preview.

Click a card to open the screenshot viewer, a dialog you can drag and resize over the Editor. The screenshot opens fit to the dialog, which keeps its size and position the next time you open it.

<Image src={screenshotsInEditor} alt="Screenshot viewer" />

The following options are available in the viewer toolbar:

* <Icon name="mdi:minus" class="inline-icon" /> **Zoom out** (<KBD>-</KBD>) and <Icon name="mdi:plus" class="inline-icon" /> **Zoom in** (<KBD>+</KBD>): Change the zoom level step by step. The current level is shown between them.
* <Icon name="mdi:fit-to-screen-outline" class="inline-icon" /> **Fit to screen** (<KBD>Space</KBD>): Fit the whole screenshot into the dialog.
* **1:1** (<KBD>1</KBD>): Display the screenshot at its actual size.
* <Icon name="mdi:crosshairs-gps" class="inline-icon" /> **Center on string** (<KBD>C</KBD>): Zoom in on the place where the string is used. When the string is tagged in several places on the same screenshot, each click moves to the next one, and a counter shows which one you are on.
* <Icon name="mdi:chevron-left" class="inline-icon" /> **Previous screenshot** (<KBD>←</KBD>) and <Icon name="mdi:chevron-right" class="inline-icon" /> **Next screenshot** (<KBD>→</KBD>): Move through all the screenshots attached to the string. A counter shows your position in the list.
* <Icon name="mdi:close" class="inline-icon" /> **Close** (<KBD>Esc</KBD>): Close the viewer.

Scroll to move around the screenshot, and hold <KBD>Ctrl</KBD> (or <KBD>⌘</KBD>) while scrolling to zoom. Click the screenshot to zoom in to 200% at that point, and hold <KBD>Alt</KBD> (or <KBD>⌥</KBD>) while clicking to zoom back out. Keyboard shortcuts work while the dialog is active.

Every string tagged on the screenshot is outlined. The string you are working on has a solid outline, and the others have dashed ones. Click a dashed outline to switch to that string, and the viewer stays on the same screenshot.

When you select another string in the list, the open viewer follows it. If the new string appears on the same screenshot, the solid outline moves to it. Otherwise, the viewer switches to that string's screenshots.
...
If you see that there is already a correct translation, vote for it by clicking the *plus* <Icon name="mdi:plus" class="inline-icon" /> sign if you like the translation, or the *minus* <Icon name="mdi:minus" class="inline-icon" /> sign if you don't think the translation is correct. Translations that receive the most positive votes will have a higher rating and will appear at the top of all available translations for the string.

<Image src={vote} alt="Vote" />

#### Automated Suggestions

AI, TM, and MT suggestions are grouped together in a single collapsible **Automated Suggestions** section (shown as **Automated Corrections** during the [Source Text Review](/enterprise/source-text-review/) step).
...
Using the **Comments** section, you can discuss the meaning of the source string or other related questions. It is recommended to use a source language of the project so other translators can understand it. Use the "@" character and a username to direct your message to the specific person. You can also mention an entire team assigned to the project by typing "@" and the team name.
...
<LinkCard
  ...
  href="/enterprise/issues/"
/>

#### Exchanging Comments with External Organizations

When collaborating with teams from other organizations, such as localization vendors, you can use shared comments to communicate directly within the Editor.

To make a comment visible to all involved organizations, select the **Shared** checkbox before posting. A notice will appear listing the organizations that will be able to see your comment, ensuring transparency in the discussion.

All comments, both private and shared, appear in a single chronological feed for easy tracking. Shared comments are clearly marked with a **Shared** label, making them easy to distinguish from internal-only discussions.

To ensure the right people are notified:

* When you create a shared **issue** (e.g., reporting a typo in the source text), project managers in all involved organizations will be notified.
* You can `@mention` a team (e.g., `@YourVendorTeam`) to notify all its members. While you can only `@mention` individual users from your own organization, this allows you to direct questions to the appropriate team.

<Aside type="tip">
  To reply directly to a comment from a user in another organization, hover over their message and click <Icon name="material-symbols:reply" class="inline-icon" /> **Reply**.

  This action automatically enables the **Shared** checkbox and adds a unique mention of the original commenter to your message (e.g., `@username:123456789:123`). This ensures a seamless conversation and that the correct person is notified across different organizations.
</Aside>

Any comments created without the **Shared** option enabled will remain private and visible only to the members of your organization.
...
When working in the Editor, you can view the TM suggestions displayed in the Automated Suggestions section and in the right sidebar found through the Search TM section. There may be situations where TM suggestions contain inaccuracies or need to be updated.

Project members with manager permissions (or higher) can edit or delete TM records directly in the Editor, both in the Automated Suggestions section and in the Search TM section, rather than having to go to the Translation Memories page in the Resources or Project Settings. This ensures that TM suggestions are updated safely and quickly, improving the overall quality of the TM.

To edit the TM suggestion, follow these steps:

<Steps>
  1. Click <Icon name="mdi:pencil" class="inline-icon" /> in the Automated Suggestions section or **Edit** in the Search TM section. <Image src={editTmSuggestions} alt="Edit TM Suggestions" />
...
The *File Context* section allows you to provide translators with additional details to help them understand how to translate a specific file. File context can be added as plain text or Markdown, either directly in the Editor or through the [File Settings](/enterprise/file-management/#file-context) by the project members with manager permissions (or higher). For other project members, file context is read-only.
...
The multilingual mode provides similar features as the side-by-side mode and allows multilingual translators and proofreaders to work with multiple languages at the same time. You can select any number of languages to work with simultaneously, up to all the languages in your project. The right panel shows the string's translations of the language you're entering a translation for or the one that was selected last. Additionally, to provide more context with fewer page turns, the Editor displays 50 strings per page in this mode.

<Image src={multilingualMode} alt="Multilingual Mode" />

Once you switch to the multilingual mode, select the languages you'd like to work with, and click **Apply**. To quickly choose all project languages with a single click, use the **Select All** button. To add new languages or remove some of the earlier selected ones, go to **Language** in the main menu, do the necessary modifications to the language list, and click **Apply**. You can also use the **Clear** button to quickly deselect all languages before making a new selection.

### Multilingual Mode (Grid)

In addition to the multilingual mode, there is a separate Multilingual (Grid) mode. It provides a spreadsheet-like layout where each language is shown in a separate column, allowing you to work with multiple languages in a compact tabular format. Like the standard Multilingual mode, it supports an unlimited number of languages simultaneously and displays 50 strings per page.
...
Additionally, to pin or hide individual columns, hover over a column header to reveal a drop-down arrow. Click the arrow and select either **Pin Column** or **Hide Column**.

You can customize the grid layout and interact with cells more efficiently using the following features:

* **Row Sizes** &ndash; Switch the table density between <Icon name="material-symbols:table-rows-outline" class="inline-icon" /> **Compact**, <Icon name="mdi:view-stream-outline" class="inline-icon" /> **Medium**, and <Icon name="material-symbols:square-outline-rounded" class="inline-icon" /> **Large** modes using the buttons in the upper-right corner. This allows you to find the most comfortable display density.
* **Inline Action Buttons** &ndash; When hovering over a translation cell, a mini-panel appears directly in that cell with quick actions: **Save**, **Cancel**, **Approve**, and **Copy Source**. This allows you to manage translations efficiently without moving your cursor to the right panel.
* **Inline Statuses and QA Checks** &ndash; String statuses and Quality Assurance (QA) check results are displayed directly inside each cell. If a translation has a QA issue, it is clearly highlighted with a <Badge text="QA" variant="danger" /> indicator next to the text, providing immediate visual feedback.
...
When using filter options in multilingual mode, the system will show strings that meet the selected criteria for at least one of the selected languages. Filters that depend on translations become available once you select at least one target language.
...
* Advanced Filter > Translations > Translations Updated
* Advanced Filter > Strings > Translation Status > Partially Translated (Plurals)
* Advanced Filter > Translations > Conditions > Translation Equals Source String
* Advanced Filter > Translations > Votes
...
- *Translation alignment* &ndash; if enabled, source words that already have translations in the project are underlined with a dashed line. Hover over an underlined word to see its previous translations and how many times each one was used. Read more about [Translation Consistency](/enterprise/translation-consistency/).
...
- *QA issues* &ndash; if enabled, inline banners with warnings will appear each time you try to save a translation with some inaccuracy (punctuation/tags/spaces mismatch, missing variables, etc.)
...
- *Auto-save translations* &ndash; if enabled, your translations are automatically saved when you navigate between strings.
...
<CardGrid>
  <LinkCard
    ...
    description="Browse the collection of available themes on the Crowdin Store."
...
Use keyboard shortcuts to take actions in the editor quickly. Check the list of keyboard shortcuts by clicking the *keyboard* <Icon name="mdi:keyboard" class="inline-icon" /> icon in the upper-right corner or selecting **Help > Keyboard Shortcuts** in the main menu. Alternatively, press <KBD>Ctrl</KBD>+<KBD>Shift</KBD>+<KBD>K</KBD> or <KBD>Shift</KBD>+<KBD>⌘</KBD>+<KBD>K</KBD>. Most of the hotkeys can be customized to your personal preferences. Click on the necessary key combination, and modify it with the help of your keyboard.
...
The path serves as a visual indicator of your location (project name, target language, file name, or workflow step (only in [projects with a workflow](/enterprise/creating-project/#projects-with-a-workflow))) and provides quick-reference information on hover:
...
<Steps>
  ...
  4. Enter the word, phrase, or sentence you want to substitute and the text to replace it with. Similarly to [string search](#string-search), you can use the *Match case* and *Exact match* options to refine the search results.
...
<Steps>
  ...
  8. In the lower-left corner, click the drop-down menu and select the preferred behavior for the translations of the source strings to be replaced. You can choose between *Keep Translations*, *Remove Approvals*, or *Delete Translations*.
...
<ReadMore>
  Read more about [Auto-Translation Queue](/enterprise/auto-translation/#auto-translation-queue).
</ReadMore>
...
You can create a custom filter for strings by clicking the **Filter strings** <Icon name="mdi:filter-variant" class="inline-icon" /> icon next to the search field and choosing **Advanced Filter**. Combine multiple filter parameters and define a custom sort order to locate specific strings faster.

#### Filter Parameters

The filter dialog groups its parameters into the following sections, and you can combine parameters from any of them to narrow down your search:

##### Search

Set where the search applies and what it looks for:

* **Search in**: Choose which part of the strings the search covers. Options include *Everything*, *Strings*, *Context*, *All translations*, *Final translations*, or *Identifier (Key)*. <Aside>In the Source Text Review step, *All translations* and *Final translations* read *All corrections* and *Final corrections*.</Aside>
* **Text**: Search for the text as typed.
* [**Verbal Expression**](#verbal-expressions): Search for a structural pattern instead of exact text. <Aside>**Text** and **Verbal Expression** are mutually exclusive, so the dialog holds one query at a time and shows which one **Filter** will replace.</Aside>

##### Strings

Filter by the properties of the source strings:
...
* [**Labels: Include All / Labels: Include Any**](#filtering-by-labels): Include strings that have all or any of the selected labels.
* [**Labels: Exclude All / Labels: Exclude Any**](#filtering-by-labels): Exclude strings that have all or any of the selected labels. <Aside>These options only appear if your project uses labels.</Aside>
* **Duplicates**: Filter strings based on their duplicate status. Options include *Master Strings*, *Duplicates Only*, *Duplicates with Shared Translations*, and *Duplicates with Own Translations*. <Aside>This option only appears if [**Duplicate Strings**](/enterprise/project-settings/import/#duplicate-strings) is configured to auto-translate duplicates. Shared translations come from the master string, while own translations are entered directly on the duplicate.</Aside>
* **String Type**: Filter by string format. Options include *Simple String*, *Plurals*, or *ICU*.
* **Translation Status**: Filter by translation status. Options include *Untranslated*, *Partially Translated (Plurals)*, and *Translated*.
* **Approval Status**: Filter strings by their approval status. Options include *Translated, Not Approved*, *Partially Approved (Plurals)*, and *Approved*.
* **Approvals Count**: Filter strings by the number of approvals they received. Options are *Greater Than* or *Less Than* a specified number.
* **QA Issues**: Filter strings based on quality assurance issues. Options include *Without QA Checks*, *QA check in progress*, *All* issues, or a specific check (for example, *Tags mismatch*, *Spelling mistakes*, *Punctuation mismatch*). <Aside>The available checks depend on the QA checks enabled for the project.</Aside>
...
* **Visibility**: Filter strings by whether translators can see them. Options are *Visible* or *Hidden*. <Aside>This option only appears if you have access to hidden strings.</Aside>

##### Translations

Filter by the translations a string already has:

* **Scope**: Define whether a filter matches any translation of a string or only the final one (the translation shown at the top of the translation list and used on download). Options are *All translations* and *Final translations*. The scope applies to **Engine**, **Translation Method**, **Votes**, **Translated By**, **Approved By**, **Translation Equals Source String**, and **Source Changed After Translation**. It becomes available once you set one of them.
...
* **Conditions**: Filter strings by conditions that you can combine with each other and with the status filters. Options include *Translation Equals Source String*, *Source Changed After Translation*, and *Translation Added After Approval*.

#### Verbal Expressions

The Advanced Filter also supports Verbal Expressions, which match strings by structural patterns, such as the placement of variables, formatting, or punctuation, rather than by exact text.

In the **Search** group, select **Verbal Expression**, enter the pattern, and set **Search in** to define where to look for it.

The search field in the Editor toolbar keeps working while a verbal expression is active, but searching there replaces the expression.

<LinkCard
  title="Expression Syntax Elements"
  description="Learn how to build and use verbal expressions with practical examples."
  href="/enterprise/expression-syntax-elements/"
/>
...
* **Labels: Include All / Labels: Include Any**: Choose the logic for including labels, then add the labels in the corresponding field.
...
* **Labels: Exclude All / Labels: Exclude Any**: Choose the logic for excluding labels, then add the labels in the corresponding field.
...
Combine these rules to narrow or expand the filtered results.
...
Set the order of the filtered strings:
