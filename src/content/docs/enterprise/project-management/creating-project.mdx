...
In Crowdin Enterprise, all projects are private by default and visible only to invited organization members (or to a vendor organization if you invite one). This gives you full control over which organization members can access each project.

To make a project publicly visible, add a [Crowdsourcing](/enterprise/crowdsourcing/) step to its workflow and publish it. Open the project, go to **Workflow > Crowdsourcing**, and click **Publish**. Published crowdsourcing projects appear on your [organization's public page](/enterprise/organization-settings/#public-organization), where they're open to volunteer translators and anyone with the link.
...
* Good if there is no need for automation (e.g., auto-translation via TM/MT, filtering content via custom code step, ordering translations and proofreading via vendors, etc.).
