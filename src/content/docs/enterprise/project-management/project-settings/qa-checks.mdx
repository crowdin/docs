...
* Custom QA checks and asynchronous external QA checks are always applied as a **Warning**. For [synchronous external QA checks](/developer/crowdin-apps-module-external-qa-check/#synchronous-checks), you choose whether they're applied as an **Error** or a **Warning**.
...
**Punctuation**: The punctuation marks in the translation don't match the ones in the source string. For many target languages, the check follows the language's own punctuation conventions: for example, a Greek question mark (;) matches a question mark in the source string, and a Chinese full stop (。) matches a full stop.
...
**Numbers mismatch**: The numbers in the translation don't match the ones in the source string or are missing from it. For many languages with their own digits, the check reads those digits and how the language groups them: for example, १२ in a Hindi translation matches 12 in the source string.
