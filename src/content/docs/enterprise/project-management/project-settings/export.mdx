...
This option works in three different ways, depending on the file format:

* **Option not applied** &ndash; Kept as-is for structured documents and layout files (e.g., MD, DOCX, HTML) because omitting content could corrupt the file structure or layout rendering.

* **Exported with empty values** &ndash; Keys or placeholders are preserved, but the translation text is left blank (e.g., JSON with nested structure, CSV).

* **Untranslated strings removed** &ndash; Strings are entirely omitted from the exported file (e.g., Android XML, macOS/iOS Strings).
