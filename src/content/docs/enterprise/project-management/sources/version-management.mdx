...
You can merge one branch into another directly from the **Branches** tab using the context menu. During the merge process, Crowdin Enterprise displays a summary of changes and allows you to select exactly which changes to accept before completing the merge.
...
<Aside>
  If a string was changed in the target branch but remained unchanged in the source branch (since it was cloned), it will not appear in the merge summary. Crowdin Enterprise considers the target branch's version to be the most up-to-date in this case, and it will be kept automatically.
</Aside>
