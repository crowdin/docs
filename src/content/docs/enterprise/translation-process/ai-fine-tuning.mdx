...
Crowdin Enterprise allows you to download datasets for external use if fine-tuning cannot be performed directly in Crowdin, if you prefer to use external tools, or if you want to validate your dataset locally before starting fine-tuning.
...
<Aside>
  Downloading datasets is available only for the Business [subscription plan](https://crowdin.com/pricing#enterprise-plans).
</Aside>
