...
To auto-translate the strings included in a translation task, follow these steps:

<Steps>
  1. Open your project and select **Tasks** on the left sidebar.
  2. Click on the name of the needed task either in the **Board** or **All tasks** section.
  3. In the task details view, click <Icon name="mdi:dots-vertical" class="inline-icon" /> in the upper-right corner and select **Auto-Translate**.
  4. In the auto-translate dialog, the task defines both the strings to process and the target language. Configure the rest of the parameters as described in the [Manual Auto-translation](#manual-auto-translation) section.
  5. Click **Auto-Translate**.
</Steps>

<Aside>
  The **Auto-Translate** option appears for **Translate by own translators** and **Translate by vendor** tasks.
</Aside>
...
- **Type:** Shows the method and specific engine or prompt used (e.g., AI, MT engine name). For auto-translations initiated from a task, the task number and title are shown as well.
...
<QuestionAnswer title="My project workflow includes a Switch Source Language step (e.g., switching from Tamil to English) followed by the TM Auto-Translation step. Which source language will be used for automatic and manual auto-translation?">
  ...
  - **Manual auto-translation** started from the project (the **Dashboard**, a language page, or **Sources > Files**) will use the initial project source language (Tamil in this case).
  - **Manual auto-translation** started within a workflow step (from the Editor opened on that step, or from a task that belongs to it) will use that step's source language (English for a step that follows the switch), unless you select a different one in **Translate from**.
  ...
  This applies to TM, MT, and AI auto-translations.
</QuestionAnswer>
