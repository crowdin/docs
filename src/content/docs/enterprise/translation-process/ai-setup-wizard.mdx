...
<Steps>
  ...
  3. Choose how to manage your credentials:
     ...
     * **Managed by Crowdin** &ndash; Ideal for testing or smaller volumes.
        <Aside>This option requires a sufficient [account balance](/enterprise/crowdin-managed-services/#adding-funds-to-account-balance). You can add funds during setup or manage your balance later in **Organization Settings > Billing > Crowdin Managed Services**.</Aside>
</Steps>
...
* **Monitor Usage and Costs** &ndash; If using a managed AI service, track your balance and billing to avoid disruptions. <ReadMore>Read more about [Using Crowdin Managed Services](/enterprise/crowdin-managed-services/#using-crowdin-managed-services).</ReadMore>
...
<QuestionAnswer title="How can I access the AI Setup Wizard if I dismissed the banner?">
  If you’ve dismissed the banner, you can still access the wizard using the following URL (replace `{your-organization-name}` with your actual Crowdin Enterprise organization name):

  ```
  https://{your-organization-name}.crowdin.com/u/ai/wizard
  ```
</QuestionAnswer>
