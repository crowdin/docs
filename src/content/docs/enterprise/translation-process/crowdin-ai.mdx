...
<Steps>
  ...
  3. In the appeared dialog, set the prompt parameters:
      ...
      * Visibility &ndash; Select projects where a prompt can be used:
          ...
          * Selected Projects &ndash; The prompt is available only in the projects you select. If no projects are selected for the Selected Projects option, the prompt will not be available in any project. <Aside>If all projects assigned to a prompt are deleted, the prompt will automatically become unavailable for any project. The organization owners and admins can always update the visibility at any time to make it available again.</Aside>
      <Aside type="tip">
