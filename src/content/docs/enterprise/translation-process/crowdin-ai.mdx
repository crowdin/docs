...
<Steps>
  ...
  3. In the appeared dialog, set the prompt parameters:
      * Select **Enabled**
      * Select the prompt type (i.e., Pre-translate, AI in editor, [Alignment](#ai-alignment) (available only for the Business [subscription plan](https://crowdin.com/pricing#enterprise-plans)), or Create custom type)
...
When enabled, AI Alignment automatically generates bilingual draft terms based on the translations made in the project, providing additional context for both human translators and AI. This feature enhances translation accuracy and consistency without requiring additional input.
