...
Once a field is created, you can work with its values on the preconfigured entities. The `List` and `Edit` methods serve different purposes:

* **Retrieve field values** – use the `List` methods to read the current field values assigned to entities. Values are returned for reference and aren't used to filter or search entities:
...
* **Update field values** – use the `Edit` methods to set or change a field's value for a specific entity:
...
Once a field is assigned to an entity, its value can be edited via API. For this, add the `fields` parameter to API calls that support fields. The `fields` parameter should contain an object with each key as the field's slug and the corresponding value as the field's value for the specific entity (e.g., `"fields": {"fieldSlug": 2}`).
