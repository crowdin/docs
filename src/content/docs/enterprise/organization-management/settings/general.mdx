...
Manage the essential details of your organization, including its name, description, logo, and public presence. You can update your organization’s name and upload a new logo, ensuring your organization’s branding is always current. The **Description** is a plain-text field that can also be used as context for AI features, and it's shown to organization members in the About organization dialog.

<ReadMore>
  See the [Organization API](/developer/enterprise/api/v2/#tag/Organization) to learn how to retrieve your organization's details, such as its name and description, programmatically.
</ReadMore>
...
Choose the default view mode (Side-by-Side, Comfortable, Multilingual, or Multilingual (Grid)) for the Editor across your organization. You can lock this setting to enforce the selected view for all users, preventing any individual changes in their Editor settings.

### Default Menu Configuration

Set the default order and visibility of the sidebar menu items for all users. The list includes both default navigation sections and any installed apps that add a navigation item. By default, users can override these defaults in their own [Customize Menu](/enterprise/account-settings/#customize-menu) settings.

To enforce the configured menu for everyone, select **Lock this setting**. When locked, users can't customize their own menu in their account settings. Content in hidden items remains safe and accessible.

To reset the menu order and visibility to their default settings, scroll down to the bottom of the list and click **Reset to defaults**.
...
In the **Billing** section, you can manage your subscription plan, track plan limits and usage, view payment history, and handle app subscriptions. You can upgrade or downgrade your plan, and review your organization's usage and invoices.

### Subscription Plan

Review your current plan: its name and the date you switched to it. The status next to the plan name shows the state of your subscription:

- **Active**: The subscription is active and set to renew.
- **Limits Exceeded**: Usage has gone over the plan limits. Upgrade the plan to keep your projects active.
- **Suspended**: The subscription won't renew. Your data is kept, and you can renew at any time.
...
- **Prepaid Balance**: The amount left from an annual payment, which Crowdin Enterprise deducts from each month. If the balance won't cover the next deduction, the amount is highlighted and shows how much is missing.
- **Balance Deduction** or **Next Charge Total**: What will be deducted from the prepaid balance, or what you'll be charged, on the next billing date. Hover over the amount to see it split into the plan, app subscriptions, and CDN Distributions.
- **Deduction Date**, **Expiration Date**, or **Next Charge Date**: When the next deduction or payment is due.
- **Renews on** or **Expiration Date**: When the plan renews or ends.
- **Yearly Price**: The approximate cost of a year on the current plan, calculated as the monthly plan and app subscription costs multiplied by 12. CDN Distributions are billed by usage and stay out of this figure, which is why it's preceded by ≈.

The **Payment Method** row names the provider your subscription is billed through. To change your billing and card details, click **Update payment details** in that row.
...
Review what your plan includes and how much of it your organization uses. The table has three columns:

- **Plan Feature**: The resource or feature, such as hosted words, managers, permission granularity, SAML, or IP allowlist.
- **Current Usage/Status**: How much your organization uses, or whether the feature is enabled.
- **Plan Limit/Availability**: The limit your plan allows, or whether the feature is available on it. Managers are broken down per role when permission granularity is enabled.

Below the table, you can compare plans and upgrade.
...
<ReadMore>
  Read more about [CDN Distributions](/enterprise/cdn-distributions/).
</ReadMore>
