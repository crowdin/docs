...
For example, there is an untranslated string `Validate your username` in the project. You generate a Cost Estimate report with the **Include auto-translated strings** option selected. This string will be included in the Cost Estimate. Then a project manager auto-translates this string via TM or MT engine and you once again generate a Cost Estimate report with the **Include auto-translated strings** option selected. This time, the auto-translated string `Validate your username` won't be included in the Cost Estimate report.
...
* **Total** &ndash; General cost estimate for all tasks.
* **Subtotals** &ndash; Cost estimate for each task:
  * **Translation** &ndash; Cost for strings requiring new or updated translations (no high–percentage match leverage).
  * **Proofreading** &ndash; Cost for reviewing translations.
  * **TM Savings** &ndash; Savings from existing translations (in TM or within the project).
  * **Weighted Words / Strings / Characters / Characters with Spaces** &ndash; Final word count for cost calculations after applying TM/internal match discounts.
...
<Steps>
  1. Select the preferred currency and the report unit (word, string, character, or characters (including spaces)).
  1. Use the available filter parameters to specify the report data you're interested in.
  1. Set your [rates](#rates-my-activity).
  1. *(Optional)* Configure the [Advanced settings](#advanced-settings-my-activity).
...
#### Advanced Settings {#advanced-settings-my-activity}

Click **Advanced settings** to expand a section with additional configuration options for your report:

* **Use category-based approval rates** (enabled by default): Treats each approval as an identical translation at the 100% match rate of its [Net Rate Scheme](#net-rate-schemes-my-activity) category (TM, MT, AI, or Other). When disabled, all approvals use the flat base proofread rate.
...
<Aside type="caution">
  When **Use category-based approval rates** is disabled, the specific **Approval** rows under the TM, MT, AI, and Other translations blocks are removed. Instead, a single, standalone **Proofreading** row is added to the bottom of the table. All approved units across the entire language section are aggregated into this row and calculated at your flat base proofreading rate.
</Aside>
...
* **User Information** &ndash; profile picture, full name, and username.
* **Total** &ndash; The total calculated cost and total time spent for your contribution during the selected time period.
* **Language Subtotals** &ndash; The report shows a breakdown of costs for each target language:
    * **Time Spent** &ndash; The total time you logged for the work in a specific language.
    * **Rate per hour** &ndash; The hourly rate applied (based on your configured [Base Rate](#base-rate-time-spent) and [Custom Rates](#custom-rates-time-spent)).
    * **Price** &ndash; The total cost calculated for the work in that language.
