...
import addDistribution from '!/enterprise/integrations/content_delivery_add_distribution.png';
import releaseDistribution from '!/enterprise/integrations/content_delivery_release_distribution.png';
import usagePrice from '!/enterprise/integrations/content_delivery_usage_price.png';
import usageStats from '!/enterprise/integrations/content_delivery_usage_statistics.png';
...
Distribution is a CDN vault that mirrors your project's translated content and is required for integration with mobile, server, desktop, and web apps. Distributions provide read-only access to your released translations via the CDN and cannot be used to modify your source files, project settings, or any other content within Crowdin Enterprise.
...
<ReadMore>
  Read more about [configuring bundles for distribution](/enterprise/bundles/).
</ReadMore>

<Aside>
  The CDN caches all the translations in release for up to 1 hour, so even after new translations are released in Crowdin Enterprise, the CDN may return them with a delay.
</Aside>
...
`{path_to_file}` will match the path used in your regular translation build. If your files don’t have export patterns including language code placeholders (e.g., `%locale%`, `%two_letters_code%`, etc.), Crowdin Enterprise automatically adds the Crowdin language code at the beginning of the path.

To see all available files and languages for your distribution, copy the Distribution Manifest URL (provided in the Crowdin Enterprise UI) and open it in your browser.
...
You can manage your Over-The-Air Content Delivery usage price and set up notifications for daily usage limits in the **Organization Settings > Billing > Over-The-Air Content Delivery**.

### OTA Usage Price

In the **Over-The-Air Content Delivery usage** section, you can see the total amount spent during the current billing cycle (one month). This includes a detailed breakdown of requests and data transfer with their respective prices. OTA usage fees are included in your primary Crowdin Enterprise subscription and charged during the next billing cycle.
...
<Steps>
  1. Open your **Organization Settings** and go to the **Billing** tab.
  2. Click **Over-The-Air Content Delivery**.
...
The **Project Statistics** section, located below the graph, provides a detailed breakdown of your OTA usage grouped by project. This table helps you track which projects are consuming the most OTA resources during the selected period.
...
<QuestionAnswer title="Can I use one SDK instance with multiple distributions?">
  A single SDK instance (Mobile or JS Client) typically connects to only one distribution hash at a time. For multiple distributions, manage separate SDK instances or reinitialize the SDK/client as needed.
</QuestionAnswer>
<QuestionAnswer title="How can I force the SDK/client to update its cache immediately?">
  Immediate client-side cache updates cannot be forced. The Crowdin SDKs and JS Client manage caching internally based on their configuration.
</QuestionAnswer>
<QuestionAnswer title="Why aren't translations for some specific UI elements updating via OTA?">
  This usually occurs if custom UI components bypass the standard localization methods supported by the SDK/client. Ensure these elements use the standard approach to receive OTA updates.
</QuestionAnswer>
