...
When you order proofreading from Crowdin Language Services, you can receive a credit return based on the quality of the initial translations. This system is designed to reward you for providing high-quality pre-translated content, whether it originates from your Translation Memory (TM), AI, or Machine Translation (MT). The better the initial translation, the less post-editing is required, and the more you save.

Here’s how it works: You pay the standard rate when creating a proofreading task. After the proofreader completes the work, they calculate the **Credit Return** based on the editing effort required for each string. This amount is then automatically refunded to your [Crowdin Managed Services balance](#managed-by-crowdin-balance) once the task is complete.

For example, if the pre-translations are of high quality and require no edits, the proofreading cost is only 30% of the standard rate.
