  ...
  <TabItem label="File-based project">
    <Steps>
      ...
      1. Set the task parameters:
          ...
          * *[Create a pending proofreading task](#sequential-tasks)* (only for Translate by own translators) &ndash; creates a separate proofreading task that starts once the translation is completed.
          * *[Preceding task](#sequential-tasks)* (only for Proofread by own proofreaders) &ndash; link the task to a previously created translation task to inherit its scope and language settings.
          ...
          * *String filters* &ndash; filter which strings should be included in the task:
            ...
            * *Include pre-translated strings only* (only for Proofread by own proofreaders) &ndash; include only strings that were previously pre-translated.
      ...
      1. *(Optional)* Use a saved [template](#task-templates) to apply language and member assignment settings, or save the current configuration for future use.
