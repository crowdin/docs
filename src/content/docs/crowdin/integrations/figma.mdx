...
<img src={previewStrings.src} alt="Figma Plugin Preview Strings" />
...
<img src={previewKeys.src} alt="Figma Plugin Preview Keys" />
...
<Aside>
  Page names in Figma can consist of only letters (a-z, A-Z), numbers (0-9), and special characters <code>-</code> <code>_</code> <code>.</code>.
</Aside>
...
<Steps>
  ...
  5. Select the needed string on the frame and hover over the <Icon name="mdi:info" class="inline-icon" /> icon in the string list to view all needed information (i.e., key, context, file it's stored in, labels) about this particular string.
