...
**Punctuation mismatch**: The punctuation marks in the translation don't match the ones in the source string, including the spacing before and after them. For many target languages, the check follows the language's own punctuation conventions: for example, a Greek question mark (;) matches a question mark in the source string, and a Chinese full stop (。) matches a full stop.
