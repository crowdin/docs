...
To view your project status, assess the effectiveness of auto-translation methods, estimate and count the translation cost, keep track of the most active members, and view historical data of previously generated reports, open your project and go to the **Reports** tab.
...
* **Repetitions (total)** or **Internal Match (total)** – Untranslated strings matching other strings within the project. Displaying depends on whether **Calculate Internal Fuzzy Matches** is enabled. Subcategories (e.g., 101%, 100%, and fuzzy) depend on your Net Rate Schemes.
  <Aside>
    ...
    - If you choose to [hide all duplicates](/project-settings/import/#hide-all-duplicates), the report won’t show any repetitions because they aren't visible for translation. The detection type (regular or strict) defines what counts as a duplicate.
  </Aside>
...
<Steps>
  1. Select the preferred currency and the report unit (words, strings, characters without spaces, or characters with spaces).
  1. Set your [rates](#rates-translation-cost).
...
* **Use category-based proofread rates** (enabled by default): Treats each approval as an identical translation at the 100% match rate of its [Net Rate Scheme](#net-rate-schemes-translation-cost) category (TM, MT, AI, or Other). When disabled, all approvals use the flat **Proofread** base rate.
...
<Image src={addCustomRatesTranslationCost} alt="Adding Custom Rates" class="width-md" />
...
* **Contributed Words / Strings / Characters / Characters with Spaces** &ndash; The raw number of units handled within that specific row type.
...
<Aside>
  When **Use category-based proofread rates** is disabled, the specific **Approval** rows under the TM, MT, AI, and Other translations blocks are removed. Instead, a single, standalone **Proofreading** row is added to the bottom of the table. All approved units across the entire language or member section are aggregated into this row and calculated at your flat base proofreading rate.
</Aside>
...
* **Date range**: Today, Yesterday, Last 7 days, Last 30 days, This month, Last month, All time, or Custom range. This filter selects the range of when the strings were auto-translated.
