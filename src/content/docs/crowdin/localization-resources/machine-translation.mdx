...
<Steps>
  ...
  3. Select **Enabled**.
  4. *(Optional)* Select the **\{Engine} managed by Crowdin** option if you'd like to use the providers managed by Crowdin (specific to DeepL Translator). In this case, you'll need to [top up your account balance](/crowdin-managed-services/#topping-up-account-balance).
  5. Enter the credentials for the selected translation engine.
  6. *(Optional)* In the **Advanced settings** section, you can configure the following options:
