...
* [AI-powered suggestions](/crowdin-ai/#using-ai-in-the-editor), [Translation Memory (TM)](/translation-memory/) suggestions, and [Machine Translation (MT)](/machine-translation/) suggestions.
...
- AI suggestions (if enabled in the [project settings](/project-settings/ai/))
...
AI, TM, and MT suggestions are grouped together in a single collapsible **Automated Suggestions** section.
...
Using the **Comments** section, you can discuss the meaning of the source string or other related questions. It is recommended to use a source language of the project so other translators can understand it. Use the "@" character and a username to direct your message to the specific person.
...
- *Translation alignment* &ndash; if enabled, source words that already have translations in the project are underlined with a dashed line. Hover over an underlined word to see its previous translations and how many times each one was used. Read more about [Translation Consistency](/translation-consistency/).
...
The path serves as a visual indicator of your location (project name, target language, or file name) and provides quick-reference information on hover:
...
<Steps>
  ...
  2. In the **Auto-Translate strings** dialog, configure the following parameters:
     ...
     * **Allow duplicate translations** – adds translations even if they duplicate existing ones. Useful when comparing AI prompts or MT engines in the [Auto-Translation Accuracy](/project-reports/#auto-translation-accuracy) report.
...
<ReadMore>
  Read more about [Auto-Translation Queue](/auto-translation/#auto-translation-queue).
</ReadMore>
...
* **Search in**: Choose which part of the strings the search covers. Options include *Everything*, *Strings*, *Context*, *All translations*, *Final translations*, or *Identifier (Key)*.
...
* **Duplicates**: Filter strings based on their duplicate status. Options include *Master Strings*, *Duplicates Only*, *Duplicates with Shared Translations*, and *Duplicates with Own Translations*. <Aside>This option only appears if [**Duplicate Strings**](/project-settings/import/#duplicate-strings) is configured to auto-translate duplicates. Shared translations come from the master string, while own translations are entered directly on the duplicate.</Aside>
...
<LinkCard
  ...
  href="/expression-syntax-elements/"
/>
