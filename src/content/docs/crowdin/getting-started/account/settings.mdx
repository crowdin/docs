...
<Aside title="Username availability">
  Each username on Crowdin is unique. If the username you want is already taken, it belongs to another account, including one that's no longer active. That username becomes available only if its owner changes their username or deletes the account.
</Aside>
...
The **Billing** tab in your **Account Settings** becomes available as soon as you subscribe to one of Crowdin's plans. Within this tab, you can manage your subscription plan, track your plan limits and usage, view payment history, handle app subscriptions, manage the Crowdin Credits balance for paid services, and set the daily balance warning threshold for CDN Distributions.

### Subscription Plan

Review your current plan: its name, the date you switched to it, and whether it's billed monthly or annually. The status next to the plan name shows the state of your subscription:

- **Active**: The subscription is running and renews automatically.
- **Limits Exceeded**: Usage has gone over the plan limits. Upgrade the plan to keep your projects active.
- **Suspended**: Billing is paused and your projects are read-only. Your data is kept, and you can renew at any time.
...
- **Prepaid Balance**: The amount left from an annual payment, which Crowdin deducts from each month. If the balance won't cover the next deduction, the amount is highlighted and shows how much is missing.
- **Next Charge Total** or **Balance Deduction**: What you'll be charged, or what will be deducted from the prepaid balance, on the next billing date. Hover over the amount to see it split into the plan, app subscriptions, and CDN Distributions.
- **Next Charge Date**, **Deduction Date**, or **Expiration Date**: When the next payment or deduction is due.
- **Renews On** or **Expiration Date**: When the plan renews or ends.
- **Yearly Price**: The approximate cost of a year on the current plan, calculated as the monthly plan and app subscription costs multiplied by 12. CDN Distributions are billed by usage and stay out of this figure, which is why it's preceded by ≈. Shown for annual plans.

If your subscription is billed through a payment provider, the **Payment Method** row names it. To change your billing and card details, click **Update Payment Details** in that row.

Use **Change Plan** to move to another plan, **Renew** to resume a suspended subscription, or **Suspend** to stop future charges.
...
See how much of your plan you've used. The table has three columns:

- **Resource**: A resource included in your plan, such as managers, projects, or source words.
- **Limit**: How much the plan allows. Resources with no limit show **Unlimited**.
- **Used**: How much of it your account currently uses.

A resource over its limit is marked with the **Over Limit** badge, and the subscription status changes to **Limits Exceeded**.
...
<Steps>
  ...
  6. *(Optional)* Configure additional options for auto-translated strings and internal fuzzy matches.
  7. Click **Save**.
</Steps>
...
In the **Customize Navigation** tab, you can manage the display of your profile navigation items, including their order and visibility. The list includes both default navigation sections and any installed apps that add a navigation item. The configured order and visibility apply to both layouts: a vertical left sidebar (when the menu is pinned) and horizontal tabs at the top of the page (when the menu is unpinned).

<ReadMore>
  Read more about managing the menu layout on your [Profile Page](/for-translators/#profile-page).
</ReadMore>

### Reordering Navigation Items

To reorder the navigation items, hover over <Icon name="material-symbols:drag-indicator" class="inline-icon" /> next to the item you want to move, then click and drag it to the preferred position in the list.

### Toggling Item Visibility

To hide or show a navigation item, clear the checkbox next to its name to hide it, or select it to make it visible.

<Aside>
  The **Projects** item is always displayed and can't be hidden. Content in hidden items remains safe and accessible.
</Aside>

### Resetting Navigation Settings

To reset the navigation order and visibility to their default settings, scroll down to the bottom of the list and click **Reset to defaults**.
