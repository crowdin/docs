...
Some agencies are integrated with Crowdin via API. When you decide to work with them, Crowdin forwards your untranslated data directly to the agency, and once completed, the translations are uploaded back to the project. Other vendors assign a dedicated manager who handles the process for you &ndash; involving translators, maintaining quality, and meeting deadlines.

To work with any vendor, create a *Translate by vendor* or *Proofread by vendor* task in your project and select the vendor you'd like to cooperate with. Before ordering, you can open the vendor's page in the Store to review their details or contact them to agree on the terms.
...
- **Auto-translate**: Generate initial AI translations that can be later reviewed by human proofreaders.
...
Configure these engines to use manual or automated auto-translation via MT. A human translator can also review those translations and do post-editing if needed. Otherwise, you can configure an MT of your choice so the machine translations will be shown in the Editor as suggestions to assist your translators.
