...
<Steps>
  ...
  3. In the appeared dialog, set the prompt parameters:
      * Select the prompt type (i.e., [Auto-Translation & AI Suggestion](#auto-translation-via-ai), [QA check](#ai-qa-check), [Advisor string context review](#advisor-string-context-review), or Create custom type)
      ...
      * Advanced mode &ndash; click **Advanced mode** to compose your prompt with a higher degree of precision. Use the following placeholders to insert context: <Aside type="tip">Click on the placeholder to copy it for further pasting into the prompt field.</Aside>
          ...
          * `%contextStrings%` &ndash; A collection of strings with their context for the AI to review during Advisor String Context Review.
...
* **Type** – The prompt's type (e.g., Auto-Translation & AI Suggestion, QA check, Custom type).
...
<Steps>
  ...
  4. Choose your credential type:
      * To use the [Managed by Crowdin service](/crowdin-credits/), select **\{Provider} managed by Crowdin**. In this case, you'll need to [add funds to your account balance](#adding-funds-to-account-balance).
  ...
  5. *(Optional)* Click **Advanced settings** to access additional options:
      ...
      * **Headers** &ndash; Add custom HTTP headers to be sent with every request to the AI provider. This is useful when a self-hosted proxy or gateway requires extra headers for authentication or routing. Enter the header name and value, then click <Icon name="mdi:plus" class="inline-icon" /> to add another one. Authorization headers are managed by Crowdin and can't be overridden, unless you select **Send only custom headers**.
          * **Send only custom headers** &ndash; Sends only your custom headers and skips the ones Crowdin adds for the provider, including authorization. Select it when your proxy or gateway handles authentication itself and the headers you enter cover everything the provider's endpoint requires.
        <Aside>
...
* **Authorization method**: The way Crowdin gets access to Vertex AI:
    * **Service account**: Uses a JSON key file that you upload in the **Service Account Key** field.
    * **Workload Identity Federation**: Uses short-lived access tokens issued by Google Cloud instead of a stored key file. Requires the **Workload Identity Federation Audience** and, optionally, the **Workload Identity Federation Service Account Email** for Crowdin to impersonate.

###### Setting Up a Service Account Key

To create the JSON key file and add it to Crowdin, follow these steps:

<Steps>
  1. In your Google Cloud project, go to **IAM & Admin > Service Accounts**.
  2. Create a new service account or select an existing one.
  3. Grant it the **Vertex AI User** role (`roles/aiplatform.user`). To use screenshots as prompt context, also grant the **Storage Admin** role (`roles/storage.admin`). Crowdin uploads the screenshots to a temporary bucket in your Google Cloud project.
...
* **Attribute condition**: `assertion.arn == 'arn:aws:sts::459083373173:assumed-role/crowdin-federated-identity/SESSION_NAME'`. This limits the trust to your Crowdin account. Crowdin shows the `SESSION_NAME` value as **Session name** under the **Workload Identity Federation Service Account Email** field.
...
When the provider is ready, enter its audience in the **Workload Identity Federation Audience** field in Crowdin:
...
<ReadMore>
  Read more about [Adding Funds to Account Balance](/crowdin-credits/#adding-funds-to-account-balance).
</ReadMore>
...
The **Usage** tab lets you control how much your account spends on AI features by setting daily and monthly budgets. You can set limits account-wide, as a per-user baseline, and for specific users. The limits apply to your project members who use AI features.
...
The overall budget for AI features across all your projects. When a limit is reached, AI requests are paused for all project members until the period resets. Below each field, a usage bar shows how much has been spent in the current period.

#### Default Per-User Limits

Applied to every project member who doesn't have custom limits. Use these fields to set a baseline for your account by raising or lowering the defaults. When a member reaches their limit, only their own AI requests are paused until the period resets.

Default per-user limits can't be higher than the account-wide limits.

#### Per-User Limits

Give specific project members custom limits that replace the default per-user limits. This is useful for power users, evaluators, or users who need tighter limits than the default.

In the **Per-user limits** section, you can view every member of your projects with the following details:

* **User**: The project member.
...
Use the **Search** field to quickly find specific users. To narrow down the list, click the **Filters** button and use the **Limits** filter:
...
<Steps>
  ...
  2. In the **Set user limits** dialog, use the **User** field to select one or more users. Only members of your projects appear in the list, and users who already have custom limits can't be selected.
  3. Choose how to limit the selected users:
     * Enter a **Daily limit**, a **Monthly limit**, or both. Leaving a field empty removes the per-user limit for that period. Leaving both empty lets those members use AI without a personal limit (the account-wide limits still apply).
...
To manage the limits of a specific user, click <Icon name="mdi:dots-horizontal" class="inline-icon" /> next to the user and select one of the following options:
...
Alternatively, just double-click the user to edit their limits.

<Aside>
  Custom limits take precedence over the default per-user limits, but the account-wide limits always apply on top. Even a member with no personal limit is still subject to the account-wide budget.
</Aside>
...
#### Advisor String Context Review {#advisor-string-context-review-account-settings}

Select a default **Advisor string context review** type prompt used to review the quality of the context provided for source strings across your projects. It flags strings whose context is missing or won't help translators understand how a string is used.
...
<Aside type="tip">
  You can also [configure an automatic AI Auto-translate](/project-settings/auto-translate/#ai-auto-translate) for new content in your project.
</Aside>
...
After configuration, you can assign a QA Check prompt as the default for all projects under your account or for individual projects.
