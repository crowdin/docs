...
<Steps>
  ...
  3. Grant it the **Vertex AI User** role (`roles/aiplatform.user`). To use screenshots as prompt context, also grant the **Storage Admin** role (`roles/storage.admin`), which lets Crowdin upload them to a temporary bucket in your Google Cloud project.
...
<Steps>
  ...
  4. Grant Vertex AI and Cloud Storage access in one of the following ways:
      * **Option A: Direct resource access** (simpler). Leave the **Workload Identity Federation Service Account Email** field empty in Crowdin.
  ...
  5. Collect the values to enter in Crowdin:
  ...
  6. In Crowdin, enter the audience in the **Workload Identity Federation Audience** field. If you used Option B, also enter the service account email in the **Workload Identity Federation Service Account Email** field.
</Steps>
...
The **Limits** tab lets you control how much your account spends on AI features by setting daily and monthly budgets. You can set limits account-wide, as a per-user baseline, and for specific users. The limits apply to your project members who use AI features.
...
Applied to every project member who doesn't have a per-user override. Use these fields to set a baseline for your account by raising or lowering the defaults. When a member reaches their limit, only their own AI requests are paused until the period resets.

Default per-user limits can't be higher than the account-wide limits.

#### Per-User Overrides

Give specific project members a custom limit that replaces the default per-user limits. This is useful for power users, evaluators, or users who need tighter limits than the default.
...
<Steps>
  ...
  2. In the **Add user override** dialog, use the **User** field to select one or more users. Only members of your projects appear in the list, and users who already have an override can't be selected.
...
To edit or remove an existing override, click <Icon name="mdi:dots-horizontal" class="inline-icon" /> next to the user and select **Edit** or **Remove**. Alternatively, just double-click the user to edit their override.

<Aside>
  A per-user override takes precedence over the default per-user limits, but the account-wide limits always apply on top. Even a member with no personal limit is still subject to the account-wide budget.
</Aside>
