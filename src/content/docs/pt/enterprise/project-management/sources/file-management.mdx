...
<Image src={filesSettings} alt="Definições de ficheiro" class="width-xl" />
...
Cada vez que atualizares o ficheiro de origem, uma nova revisão é criada. To restore your file to the previous revision, click the number on the needed file in the *Revision* column. In the **Revisions** tab in the right sidebar, hover over the needed revision and click **Restore** next to it.
...
Still, if you need to create branches manually, make sure to upload the master branch first, as all the others will be considered the feature branches. Usually, branches in Crowdin Enterprise are created automatically if you use GitHub, GitLab, or other VCS integration.
