...
Os Webhooks notificam e ajudam-te a recolher informações sobre os principais eventos que acontecem no teu projeto no Crowdin, como traduções concluídas ou revisão. After you configure a webhook, Crowdin Enterprise will start sending POST or GET requests with data to the webhook URL via HTTP.
